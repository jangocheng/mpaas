angular.module('app').controller('06e76d73', ['$scope', '$http', '$state', '$rootScope', '$compile', '$translate',
	function($scope, $http, $state, $rootScope, $compile, $translate) {
		...
		$scope.initLocale = function() {
			app.getJsonObject("5540b2eb/i18n/scriptCommandManage/" + $translate.use() + ".json", function(r) {
				if(r) {
					$scope.i18n = r;
				}
		...
		//初始化
		$scope.init = function() {
			var sr = localStorage.getItem("dccd3a07-492f-4fb3-a8ef-1e8148223dca");
		...
		$scope.search = function(clean) {
			if(clean) {
				//清理搜索条件
				localStorage.removeItem("dccd3a07-492f-4fb3-a8ef-1e8148223dca");
			...
			localStorage.setItem("dccd3a07-492f-4fb3-a8ef-1e8148223dca", JSON.stringify($scope.selectRequest));
			$scope.select();
		};
		//更新状态
		$scope.statusUpdate = function(row) {
			app.get($scope, "e75bf1f6-7a03-4657-85da-793e7626fbdf?id=" + row.id + "&status=" + (row.status == 1 ? 0 : 1), function(r) {
		...
		$scope.batchDelete = function() {
			app.batchDelete($scope, "408fe258-31a2-4ead-82b1-a9f140db9a73", app.getPageIds($scope), function(r) {
				$scope.toaster.popInfo({
					type: 'success',
		...
		$scope.delete = function(row) {
			app.get($scope, "0c41bdb7-cf2f-487b-8941-e4f6803b487c?id=" + row.id, function(r) {
				$scope.toaster.popInfo({
					type: 'success',
					title: $scope.i18n.delete.name,
		...
		$scope.select = function() {
			...
			app.select($scope, "c35fd116-1f35-4f61-94cd-bcd1ba2843b4", obj, function(r) {
		...
		$scope.edit = function(row) {
			if(row == null) {
				...
				o.content = $scope.data.content;
				o.description = $scope.data.description;
				app.edit($scope, "7e9863de-c58d-48a4-a91b-ad7195541767", o, function(r) {
					$scope.toaster.popInfo({
						type: 'info',
						title: $scope.i18n.edit.name,
		...
		$scope.add = function() {
			...
			app.add($scope, "4504eed5-cf92-446d-913c-b4681fe0bd93", {
		...
		$scope.get = function(row, fun) {
			app.get($scope, "29fcb051-2180-48ba-8534-d8136f8a1427?id=" + row.id, function(r) {
				//回调数据
				fun(r.data.data);
			});
		};
