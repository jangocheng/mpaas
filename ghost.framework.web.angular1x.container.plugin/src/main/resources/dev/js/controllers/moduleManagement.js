app.controller('0154db82-1925-4185-912a-5c730b9e31a2', ['$scope', '$http', '$state', '$rootScope', '$compile', '$modal', '$log',
	function($scope, $http, $state, $rootScope, $compile, $modal, $log) {
		...
		$scope.currentModule = {};
		$scope.pluginTableContent = false;
		...
		$scope.init = function() {
			...
			var sr = localStorage.getItem("8dc05127-b0de-4517-93f2-64ced648875f");
			...
			$scope.selectRequest.internalPlugin = '0';
			$scope.select();
		};
		...
		$scope.search = function(clean) {
			if(clean) {
				//清理搜索条件
				localStorage.removeItem("8dc05127-b0de-4517-93f2-64ced648875f");
			...
			localStorage.setItem("8dc05127-b0de-4517-93f2-64ced648875f", JSON.stringify($scope.selectRequest));
			$scope.select();
		};
		//更新状态
		...
		//插件管理
		$scope.plugin = function(row){
			$scope.currentModule = row;
			$scope.pluginTableContent = true;
