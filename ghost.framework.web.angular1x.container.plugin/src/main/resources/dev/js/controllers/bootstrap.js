...
app.controller('ModalCtrl', ['$scope', '$modal', '$log', function($scope, $modal, $log) {
	...
	$scope.init = function(url) {
		$scope.url = url;
		$scope.$parent.group.selectRequest.status = '-1';
	};
	$scope.open = function(size) {
		$scope.$parent.group.select(); //获取数据
...
//分页控件
app.controller('PaginationCtrl', ['$scope', '$log', function($scope, $log) {
	$scope.setPage = function(pageNo) {
		console.log("pageNo:" + pageNo);
	};
	$scope.pageChanged = function() {
		$log.info('当前页码: ' + $scope.$parent.selectRequest.start);
	};
}]);
