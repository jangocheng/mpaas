...
package ghost.framework.undertow.web.context;

import ghost.framework.util.ResourceUtils;
import ghost.framework.web.context.server.Ssl;
import ghost.framework.web.context.server.SslStoreProvider;
...
public class SslBuilderCustomizer implements UndertowBuilderCustomizer {
	...
	public SslBuilderCustomizer(ClassLoader classLoader, int port, InetAddress address, Ssl ssl, SslStoreProvider sslStoreProvider) {
	...
	private KeyStore loadStore(String type, String provider, String resource, String password) throws Exception {
		...
		try {
			URL url = ResourceUtils.getURL(classLoader, resource);
