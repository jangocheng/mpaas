...
class LettuceStreamCommands implements RedisStreamCommands {
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands#xAck(byte[], byte[], java.lang.String[])
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands#xAdd(byte[], MapRecord, XAddOptions)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands#xClaimJustId(byte[], java.lang.String, java.lang.String, ghost.framework.data.redis.connection.RedisStreamCommands.XClaimOptions)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands#xClaim(byte[], java.lang.String, java.lang.String, ghost.framework.data.redis.connection.RedisStreamCommands.XClaimOptions)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands#xDel(byte[], java.lang.String[])
	 */
	@Override
	...
	public StreamInfo.XInfoStream xInfo(byte[] key) {
		...
		try {
			if (isPipelined()) {
				pipeline(connection.newLettuceResult(getAsyncConnection().xinfoStream(key), StreamInfo.XInfoStream::fromList));
				return null;
			}
			if (isQueueing()) {
				transaction(connection.newLettuceResult(getAsyncConnection().xinfoStream(key), StreamInfo.XInfoStream::fromList));
				return null;
			}
			return StreamInfo.XInfoStream.fromList(getConnection().xinfoStream(key));
		} catch (Exception ex) {
			throw convertLettuceAccessException(ex);
		}
	}
	...
	public StreamInfo.XInfoGroups xInfoGroups(byte[] key) {
		...
		try {
			if (isPipelined()) {
				pipeline(connection.newLettuceResult(getAsyncConnection().xinfoGroups(key), StreamInfo.XInfoGroups::fromList));
				return null;
			}
			if (isQueueing()) {
				transaction(connection.newLettuceResult(getAsyncConnection().xinfoGroups(key), StreamInfo.XInfoGroups::fromList));
				return null;
			}
			return StreamInfo.XInfoGroups.fromList(getConnection().xinfoGroups(key));
		} catch (Exception ex) {
			throw convertLettuceAccessException(ex);
		}
	}
		...
		try {
			if (isPipelined()) {
				pipeline(connection.newLettuceResult(getAsyncConnection().xinfoConsumers(key, binaryGroupName),
						it -> StreamInfo.XInfoConsumers.fromList(groupName, it)));
				return null;
			}
			if (isQueueing()) {
			...
			return StreamInfo.XInfoConsumers.fromList(groupName, getConnection().xinfoConsumers(key, binaryGroupName));
		} catch (Exception ex) {
			throw convertLettuceAccessException(ex);
		}
	}
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands#xPending(byte[], java.lang.String, ghost.framework.data.redis.connection.RedisStreamCommands.XPendingOptions)
	 */
	@Override
