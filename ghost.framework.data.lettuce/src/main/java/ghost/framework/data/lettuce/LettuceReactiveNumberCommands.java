...
import ghost.framework.data.redis.connection.ReactiveNumberCommands;
...
class LettuceReactiveNumberCommands implements ReactiveNumberCommands {
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.ReactiveRedisConnection.ReactiveNumberCommands#incr(org.reactivestreams.Publisher)
	 */
	@Override
	public Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> incr(Publisher<ReactiveRedisConnection.KeyCommand> commands) {
		...
		return connection.execute(cmd -> Flux.from(commands).concatMap(command -> {
			...
			return cmd.incr(command.getKey()).map(value -> new ReactiveRedisConnection.NumericResponse<>(command, value));
		}));
	}

	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.ReactiveRedisConnection.ReactiveNumberCommands#incrBy(org.reactivestreams.Publisher, java.util.function.Supplier)
	 */
	@Override
	public <T extends Number> Flux<ReactiveRedisConnection.NumericResponse<IncrByCommand<T>, T>> incrBy(Publisher<IncrByCommand<T>> commands) {
		...
		return connection.execute(cmd -> Flux.from(commands).concatMap(command -> {
			...
			return result.map(val -> NumberUtils.convertNumberToTargetClass(val, incrBy.getClass()))
					.map(res -> new ReactiveRedisConnection.NumericResponse<>(command, (T) res));
		}));
	}

	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.ReactiveRedisConnection.ReactiveNumberCommands#decr(org.reactivestreams.Publisher)
	 */
	@Override
	public Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> decr(Publisher<ReactiveRedisConnection.KeyCommand> commands) {
		return connection.execute(cmd -> Flux.from(commands).concatMap(command -> {
			...
			return cmd.decr(command.getKey()).map(value -> new ReactiveRedisConnection.NumericResponse<>(command, value));
		}));
	}

	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.ReactiveRedisConnection.ReactiveNumberCommands#decrBy(org.reactivestreams.Publisher, java.util.function.Supplier)
	 */
	@Override
	public <T extends Number> Flux<ReactiveRedisConnection.NumericResponse<DecrByCommand<T>, T>> decrBy(Publisher<DecrByCommand<T>> commands) {
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.ReactiveNumberCommands#hIncrBy(org.reactivestreams.Publisher)
	 */
	@Override
	public <T extends Number> Flux<ReactiveRedisConnection.NumericResponse<HIncrByCommand<T>, T>> hIncrBy(Publisher<HIncrByCommand<T>> commands) {
		...
		return connection.execute(cmd -> Flux.from(commands).concatMap(command -> {
			...
			return result.map(val -> NumberUtils.convertNumberToTargetClass(val, incrBy.getClass()))
					.map(value -> new ReactiveRedisConnection.NumericResponse<>(command, (T) value));
		}));
	}
