...
@Application
@Entity
@Table(name= "repository_module_attributes",
        indexes = {
                @Index(name = "uk", columnList = "repository_module_attributes_id", unique = true),
                @Index(name = "pk", columnList = "repository_module_attributes_id,repository_module_id,name")
        })
@Cacheable
@org.hibernate.annotations.Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
public class RepositoryModuleAttributesEntity implements Serializable {

    private static final long serialVersionUID = 5616116461367468930L;
    /**
     * 插件属性id
     */
    @Id
    @Column(name = "repository_module_attributes_id", updatable = false, nullable = false, length = 36, columnDefinition = "char(36)")
