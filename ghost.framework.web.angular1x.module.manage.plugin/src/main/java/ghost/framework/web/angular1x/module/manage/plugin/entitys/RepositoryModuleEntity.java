...
@Application
@Entity
@Table(name= "repository_module",
        indexes = {
                @Index(name = "uk", columnList = "repository_module_id", unique = true),
                @Index(name = "pk", columnList = "repository_module_id,group_id,artifact_id,version,status,description,create_time")
        })
@Cacheable
@org.hibernate.annotations.Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
public class RepositoryModuleEntity implements Serializable {
     ...
     */
    @JsonIgnore
    @OneToOne(targetEntity = RepositoryModuleAttributesEntity.class, fetch = FetchType.LAZY)
    @JoinColumn(name = "repository_module_id", referencedColumnName = "repository_module_id", insertable = false, updatable = false,
            //不创建外键约束
            foreignKey = @ForeignKey(name = "none", value = ConstraintMode.NO_CONSTRAINT))
    private RepositoryModuleAttributesEntity attributesEntity;

    /**
     * 模块数据
     */
    @JsonIgnore
    @OneToOne(targetEntity = RepositoryModuleDataEntity.class, fetch = FetchType.LAZY)
    @JoinColumn(name = "repository_module_id", referencedColumnName = "repository_module_id", insertable = false, updatable = false,
            //不创建外键约束
            foreignKey = @ForeignKey(name = "none", value = ConstraintMode.NO_CONSTRAINT))
    private RepositoryModuleDataEntity dataEntity;
}
