 ...
 * @Description:绑定 {@link PreDestroy} JSR-250标准注解之 @PreDestroy注释事件工厂
 * @Date: 2020/2/21:13:45
 */
@MethodAnnotationEventFactory(lifeCycle = AnnotationTag.AnnotationLifeCycle.UnLoader)
public class JSR250MethodPreDestroyAnnotationBeanFactory<O extends ICoreInterface, T extends Object, E extends IMethodBeanTargetHandle<O, T, Method>>
        implements MethodAnnotationBeanFactory<O, T, E> {
    public JSR250MethodPreDestroyAnnotationBeanFactory(@Autowired IApplication app) {
        this.app = app;
    }

    @Override
    public String toString() {
        return "JSR250MethodPreDestroyAnnotationBeanFactory{" +
     ...
     * 类型注释工厂注释
     */
    private Class<? extends Annotation> annotation = PreDestroy.class;
    ...
    public void loader(E event) {
        ...
        //获取注释
        PreDestroy preDestroy = this.getAnnotation(event);
        //判断是否无效处理引发错误
        try {
