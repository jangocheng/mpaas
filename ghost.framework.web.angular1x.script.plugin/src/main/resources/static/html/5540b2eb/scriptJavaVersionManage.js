angular.module('app').controller('ed624a27', ['$scope', '$http', '$state', '$rootScope', '$compile', '$translate','$rootScope',
	function($scope, $http, $state, $rootScope, $compile, $translate,$rootScope) {
		...
		$scope.initLocale = function() {
			app.getJsonObject("5540b2eb/i18n/scriptJavaVersionManage/" + $translate.use() + ".json", function(r) {
				if(r) {
					$scope.i18n = r;
				}
		...
		//初始化
		$scope.init = function() {
			var sr = localStorage.getItem("610fe969-4027-46c2-a64e-21cabf6ae7ed");
		...
		$scope.search = function(clean) {
			if(clean) {
				//清理搜索条件
				localStorage.removeItem("610fe969-4027-46c2-a64e-21cabf6ae7ed");
				$scope.selectRequest.key = "";
				$scope.selectRequest.start = 1;
			}
			localStorage.setItem("610fe969-4027-46c2-a64e-21cabf6ae7ed", JSON.stringify($scope.selectRequest));
			$scope.select();
		};
		//删除全部选中
		$scope.batchDelete = function() {
			app.batchDelete($scope, "77927f40-3860-42ea-a978-c818f5ca6dd6", app.getPageIds($scope), function(r) {
		...
		$scope.delete = function(row) {
			app.get($scope, "a7c92681-37c6-4d8c-a030-383791f2d37b?id=" + row.id, function(r) {
				$scope.toaster.popInfo({
					type: 'success',
					title: $scope.i18n.delete.name,
		...
		$scope.select = function() {
			...
			app.select($scope, "db3fd21d-6ffd-47d4-94ab-88c7c416c2bf", obj, function(r) {
		...
		$scope.edit = function(row) {
			if(row == null) {
				...
				app.edit($scope, "c3b4179c-e7d3-4c3f-82ad-5a231e1dff31", o, function(r) {
					$scope.toaster.popInfo({
						type: 'info',
		...
		$scope.add = function() {
			...
			app.add($scope, "5f93bd2c-095c-4af3-b04d-26f74c6fe391", {
		...
		$scope.get = function(row, fun) {
			app.get($scope, "0e317f1e-045a-45d4-8666-89d4700dbb12?id=" + row.id, function(r) {
				//回调数据
				fun(r.data.data);
			});
		};
