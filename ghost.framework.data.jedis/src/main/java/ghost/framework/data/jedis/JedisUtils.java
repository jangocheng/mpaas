...
public abstract class JedisUtils {
	 ...
	 */
	@Deprecated
	static Set<RedisZSetCommands.Tuple> convertJedisTuple(Set<redis.clients.jedis.Tuple> tuples) {
	...
	static ListPosition convertPosition(RedisListCommands.Position where) {
		Assert.notNull(where, "list positions are mandatory");
		return (RedisListCommands.Position.AFTER.equals(where) ? ListPosition.AFTER : ListPosition.BEFORE);
	}
