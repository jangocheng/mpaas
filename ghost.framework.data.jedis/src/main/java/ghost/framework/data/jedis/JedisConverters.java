...
import ghost.framework.data.redis.connection.convert.*;
...
abstract public class JedisConverters extends Converters {
	...
	private static final SetConverter<redis.clients.jedis.Tuple, RedisZSetCommands.Tuple> TUPLE_SET_TO_TUPLE_SET;
	...
	private static final ListConverter<redis.clients.jedis.Tuple, RedisZSetCommands.Tuple> TUPLE_LIST_TO_TUPLE_LIST_CONVERTER;
	...
	static {
		...
		SET_OPTION_TO_COMMAND_OPTION_CONVERTER = new Converter<RedisStringCommands.SetOption, byte[]>() {
			...
			public byte[] convert(RedisStringCommands.SetOption source) {
				if (source == RedisStringCommands.SetOption.UPSERT) {
					return new byte[0];
				}
				if (source == RedisStringCommands.SetOption.SET_IF_ABSENT) {
					return NX;
				}
				if (source == RedisStringCommands.SetOption.SET_IF_PRESENT) {
					return XX;
				}
				/*
				switch (source) {
...
*/
	...
	public static BitOP toBitOp(RedisStringCommands.BitOperation bitOp) {
		if (bitOp == RedisStringCommands.BitOperation.AND) {
			return BitOP.AND;
		}
		if (bitOp == RedisStringCommands.BitOperation.OR) {
			return BitOP.OR;
		}
		if (bitOp == RedisStringCommands.BitOperation.NOT) {
			return BitOP.NOT;
		}
		if (bitOp == RedisStringCommands.BitOperation.XOR) {
			return BitOP.XOR;
		}
		throw new IllegalArgumentException();
		/*
		switch (bitOp) {
			case RedisStringCommands.BitOperation.AND:
				return BitOP.AND;
			case RedisStringCommands.BitOperation.OR:
				return BitOP.OR;
			case RedisStringCommands.BitOperation.NOT:
				return BitOP.NOT;
			case RedisStringCommands.BitOperation.XOR:
				return BitOP.XOR;
			default:
				throw new IllegalArgumentException();
		}*/
	 ...
	 * @since 1.8
	 */
	public static Converter<List<GeoRadiusResponse>, GeoResults<RedisGeoCommands.GeoLocation<byte[]>>> geoRadiusResponseToGeoResultsConverter(
			Metric metric) {
	...
	/**
	 * Convert {@link RedisGeoCommands.GeoRadiusCommandArgs} into {@link GeoRadiusParam}.
	 *
	 * @param source
	 * @return
	 * @since 1.8
	...
	enum GeoResultsConverterFactory {
		...
		private static class GeoResultsConverter
			...
			public GeoResults<RedisGeoCommands.GeoLocation<byte[]>> convert(List<GeoRadiusResponse> source) {
				...
				Converter<GeoRadiusResponse, GeoResult<RedisGeoCommands.GeoLocation<byte[]>>> converter = GeoResultConverterFactory.INSTANCE
						.forMetric(metric);
	...
	enum GeoResultConverterFactory {
		...
		private static class GeoResultConverter
			...
			@Override
			public GeoResult<RedisGeoCommands.GeoLocation<byte[]>> convert(redis.clients.jedis.GeoRadiusResponse source) {
