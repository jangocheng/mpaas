...
class JedisClusterStringCommands implements RedisStringCommands {
	...
	public List<byte[]> mGet(byte[]... keys) {
		...
		return connection.getClusterCommandExecutor()
				.executeMultiKeyCommand((JedisClusterConnection.JedisMultiKeyClusterCommandCallback<byte[]>) BinaryJedis::get, Arrays.asList(keys))
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStringCommands#bitPos(byte[], boolean, ghost.framework.data.Range)
	 */
	@Override
	public Long bitPos(byte[] key, boolean bit, Range<Long> range) {
		...
		args.add(Converters.toBit(bit));
		if (range.getLowerBound().isBounded()) {
			args.add(range.getLowerBound().getValue().map(Converters::toBytes).get());
		}
		if (range.getUpperBound().isBounded()) {
			args.add(range.getUpperBound().getValue().map(Converters::toBytes).get());
		}
