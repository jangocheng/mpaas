...
import ghost.framework.data.redis.connection.RedisSetCommands;
...
import ghost.framework.data.redis.connection.util.ByteArraySet;
...
class JedisClusterSetCommands implements RedisSetCommands {
	...
	public Set<byte[]> sInter(byte[]... keys) {
		...
		Collection<Set<byte[]>> resultList = connection.getClusterCommandExecutor()
				.executeMultiKeyCommand(
						(JedisClusterConnection.JedisMultiKeyClusterCommandCallback<Set<byte[]>>) (client, key) -> client.smembers(key),
						Arrays.asList(keys))
				.resultsAsList();
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisSetCommands#sScan(byte[], ghost.framework.data.redis.core.ScanOptions)
	 */
	@Override
