angular.module('app').controller('699436c1', ['$scope', '$http', '$state', '$rootScope', '$compile', '$translate',
	function($scope, $http, $state, $rootScope, $compile, $translate) {
		...
		$scope.initLocale = function() {
			app.getJsonObject("3e1e51ed/i18n/sshRegionManage/" + $translate.use() + ".json", function(r) {
				if(r) {
					$scope.i18n = r;
				}
		...
		//初始化
		$scope.init = function() {
			var sr = localStorage.getItem("1ed6e31c-1c2b-4648-9fcb-db84c11db6d8");
		...
		$scope.search = function(clean) {
			if(clean) {
				//清理搜索条件
				localStorage.removeItem("1ed6e31c-1c2b-4648-9fcb-db84c11db6d8");
			...
			localStorage.setItem("1ed6e31c-1c2b-4648-9fcb-db84c11db6d8", JSON.stringify($scope.selectRequest));
			$scope.select();
		};
		//更新状态
		$scope.statusUpdate = function(row) {
			app.get($scope, "8b12a92f-063f-4b84-aad5-c75969e33f2f?id=" + row.id + "&status=" + (row.status == 1 ? 0 : 1), function(r) {
		...
		$scope.batchDelete = function() {
			app.batchDelete($scope, "5d3a7b91-a1a8-4105-b8ee-b59b5ed10ee2", app.getPageIds($scope), function(r) {
				$scope.toaster.popInfo({
					type: 'success',
		...
		$scope.delete = function(row) {
			app.get($scope, "989ee3ac-efff-4b9d-b0ad-632bca086255?id=" + row.id, function(r) {
				$scope.toaster.popInfo({
					type: 'success',
					title: $scope.i18n.delete.name,
		...
		$scope.select = function() {
			...
			app.select($scope, "a59a7dbc-0d5a-4f31-b7e0-588451284681", obj, function(r) {
		...
		$scope.edit = function(row) {
			if(row == null) {
				...
				app.edit($scope, "1155e632-0bb7-4c63-b9c8-b30250f9a3bd", o, function(r) {
					$scope.toaster.popInfo({
						type: 'info',
						title: $scope.i18n.edit.name,
		...
		$scope.add = function() {
			...
			app.add($scope, "3db2970f-bf50-474f-9f9f-ef965846445f", {
		...
		$scope.get = function(row, fun) {
			app.get($scope, "fbd9726c-4a8b-4b3c-bfa4-90be4cb663c8?id=" + row.id, function(r) {
				//回调数据
				fun(r.data.data);
			});
		};
