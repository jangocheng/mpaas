...
package ghost.framework.data.transaction.jta.websphere.uow;
import com.ibm.websphere.uow.UOWSynchronizationRegistry;
import com.ibm.wsspi.uow.*;
import ghost.framework.beans.annotation.constraints.Nullable;
import ghost.framework.transaction.CallbackPreferringPlatformTransactionManager;
import ghost.framework.transaction.TransactionCallback;
import ghost.framework.transaction.TransactionFactory;
...
public class WebSphereUowTransactionManager extends JtaTransactionManager
	 ...
	 * @see ghost.framework.jca.endpoint.AbstractMessageEndpointFactory
	...
	public <T> T execute(@Nullable TransactionDefinition definition, TransactionCallback<T> callback)
		...
		if (existingTx) {
			...
			if (pb == TransactionDefinition.PROPAGATION_SUPPORTS ||
				...
				joinTx = true;
				newSynch = (getTransactionSynchronization() != AbstractPlatformTransactionManager.SYNCHRONIZATION_NEVER);
			}
			else if (pb == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
				uowType = UOWSynchronizationRegistry.UOW_TYPE_LOCAL_TRANSACTION;
				newSynch = (getTransactionSynchronization() == AbstractPlatformTransactionManager.SYNCHRONIZATION_ALWAYS);
			}
			else {
				newSynch = (getTransactionSynchronization() != AbstractPlatformTransactionManager.SYNCHRONIZATION_NEVER);
			}
