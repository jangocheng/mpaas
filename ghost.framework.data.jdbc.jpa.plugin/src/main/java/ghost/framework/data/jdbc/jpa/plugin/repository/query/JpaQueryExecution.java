...
import ghost.framework.context.converter.ConfigurableConversionService;
...
import ghost.framework.data.commons.domain.PageableExecutionUtils;
import ghost.framework.data.commons.domain.Slice;
import ghost.framework.data.commons.domain.SliceImpl;
import ghost.framework.data.jdbc.jpa.plugin.provider.PersistenceProvider;
import ghost.framework.data.jdbc.jpa.plugin.repository.support.SurroundingTransactionDetectorMethodInterceptor;
...
//import ghost.framework.data.repository.core.support.SurroundingTransactionDetectorMethodInterceptor;
 ...
 * {@link ghost.framework.data.repository.query.QueryMethod} a {@link AbstractStringBasedJpaQuery} can be executed
 * in various flavors.
 *
...
public abstract class JpaQueryExecution {
	...
	private static final ConverterContainer CONVERSION_SERVICE;

	static {

		ConfigurableConversionService conversionService = null;//new DefaultConverterContainer();

		conversionService.add(JpaResultConverters.BlobToByteArrayConverter.INSTANCE);
//		conversionService.remove(Collection.class, Object.class);
	...
	static class SlicedExecution extends JpaQueryExecution {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.jpa.repository.query.JpaQueryExecution#doExecute(ghost.framework.data.jpa.repository.query.AbstractJpaQuery, java.lang.Object[])
		 */
		@Override
	...
	/**
	 * Executes the {@link AbstractStringBasedJpaQuery} to return a {@link Page} of
	 * entities.
	 */
	static class PagedExecution extends JpaQueryExecution {
		...
		private long count(AbstractJpaQuery repositoryQuery, JpaParametersParameterAccessor accessor) {
			...
			return (totals.size() == 1 ? (long)CONVERSION_SERVICE.convert(totals.get(0), Long.class) : totals.size());
		}
	}
	...
	static class StreamExecution extends JpaQueryExecution {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.jpa.repository.query.JpaQueryExecution#doExecute(ghost.framework.data.jpa.repository.query.AbstractJpaQuery, JpaParametersParameterAccessor)
		 */
		@Override
...
//				conversionService.remove(Object.class, optionalType);
