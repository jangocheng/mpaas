...
//import ghost.framework.data.jpa.repository.support.JpaRepositoryFactory;
...
import ghost.framework.data.jdbc.jpa.plugin.repository.support.JpaRepositoryFactory;
...
class JpaRepositoryBean<T> extends CdiRepositoryBean<T> {
	...
	JpaRepositoryBean(BeanManager beanManager, Bean<EntityManager> entityManagerBean, Set<Annotation> qualifiers,
			Class<T> repositoryType, Optional<CustomRepositoryImplementationDetector> detector, ICoreInterface coreInterface) {
	...
	protected T create(CreationalContext<T> creationalContext, Class<T> repositoryType) {
		...
		return create(() -> new JpaRepositoryFactory(coreInterface, entityManager.getEntityManagerFactory(), entityManager), repositoryType);
	}
}
