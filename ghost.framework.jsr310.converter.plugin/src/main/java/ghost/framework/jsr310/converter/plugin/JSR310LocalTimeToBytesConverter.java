...
import ghost.framework.jsr310.converter.LocalTimeToBytesConverter;
 ...
 * @Description: {@link LocalTime} to {@link byte[]}
 * @Date: 2020/6/26:1:22
 */
@ConverterFactory
public class JSR310LocalTimeToBytesConverter implements LocalTimeToBytesConverter<LocalTime, byte[]> {
    public JSR310LocalTimeToBytesConverter(@Autowired(required = false) ICoreInterface domain) {
        this.domain = domain;
    }
    ...
    public boolean equals(Object o) {
        ...
        JSR310LocalTimeToBytesConverter that = (JSR310LocalTimeToBytesConverter) o;
    ...
    public String toString() {
        return "JSR310LocalTimeToBytesConverter{" +
    ...
    @Override
    public boolean canConvert(LocalTime source) {
        try {
            //判断是否可以转换
            if (source == null) {
                return false;
    ...
    @Override
    public byte[] convert(LocalTime source, String characterEncoding) throws ConverterException {
        try {
    ...
    @Override
    public byte[] convert(LocalTime source) throws ConverterException {
