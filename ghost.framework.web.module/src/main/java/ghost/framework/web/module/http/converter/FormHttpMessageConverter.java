...
import ghost.framework.web.module.http.HttpEntity;
import ghost.framework.web.module.http.StreamingHttpOutputMessage;
 ...
 * @author Sam Brannen
 * @since 3.0
 * @see ghost.framework.http.converter.support.AllEncompassingFormHttpMessageConverter
 * @see ghost.framework.util.MultiValueMap
 */
public class FormHttpMessageConverter implements HttpMessageConverter<MultiValueMap<String, ?>> {
	...
	protected String serializeForm(MultiValueMap<String, Object> formData, Charset charset) {
		...
		formData.forEach((name, values) -> {
				if (name == null) {
					Assert.isFalse(CollectionUtils.isEmpty(values), "Null name in form data: " + formData);
					return;
				}
	 ...
	 * <p>The default implementation returns {@link IResource#getFilename()} if the part is a
	...
	protected String getFilename(Object part) {
		if (part instanceof IResource) {
			IResource resource = (IResource) part;
	...
	/**
	 * Implementation of {@link ghost.framework.http.HttpOutputMessage} used
