...
import ghost.framework.web.context.bens.factory.IClassRestControllerAnnotationBeanFactory;
 ...
 * @Description:默认处理 {@link RestController} 注释事件工厂类
 * @Date: 13:21 2020/1/31
 */
@ClassAnnotationBeanFactory(single = true)
public class ClassRestControllerAnnotationBeanFactory
        <
        ...
        implements IClassRestControllerAnnotationBeanFactory<O, T, E, V> {

    /**
     * 注释类型
     */
    private final Class<? extends Annotation> annotation = RestController.class;
    ...
    public void loader(E event) {
        ...
        //获取注入注释对象
        RestController controller = this.getAnnotation(event);
        //构建类型实例
        //判断是否使用代理创建控制器实例
        if (controller.proxy()) {
