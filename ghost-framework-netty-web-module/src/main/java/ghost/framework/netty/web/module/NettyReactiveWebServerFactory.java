...
import ghost.framework.web.module.http.HttpStatus;
...
import ghost.framework.netty.web.module.http.client.reactive.ReactorResourceFactory;
import ghost.framework.netty.web.module.http.server.reactive.ReactorHttpHandlerAdapter;
...
public class NettyReactiveWebServerFactory extends AbstractReactiveWebServerFactory {
	...
	@Bean
	@Order(0)
	public HttpHandler echoHandler(){
	...
	@Order(1)
	@Override
	public WebServer getWebServer(@Autowired HttpHandler httpHandler) {
		HttpServer httpServer = createHttpServer();
		ReactorHttpHandlerAdapter handlerAdapter = new ReactorHttpHandlerAdapter(httpHandler);
		NettyWebServer webServer = new NettyWebServer((ClassLoader) this.module.getClassLoader(), httpServer, handlerAdapter, this.lifecycleTimeout);
	...
	private HttpServer createHttpServer() {
		...
		if (getSsl() != null && getSsl().isEnabled()) {
			SslServerCustomizer sslServerCustomizer = new SslServerCustomizer((ClassLoader) this.module.getClassLoader(), getSsl(), getHttp2(), getSslStoreProvider());
