...
import ghost.framework.aop.aspectj.*;
import ghost.framework.aop.support.DefaultBeanFactoryPointcutAdvisor;
...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
	...
	/**
	 * Configures the auto proxy creator needed to support the {@link IBeanDefinition BeanDefinitions}
	 ...
	 * {@link ghost.framework.aop.Advisor} and any resulting {@link ghost.framework.aop.Pointcut}
	...
	private void parseAdvisor(Element advisorElement, ParserContext parserContext) {
		...
		try {
			...
			if (pointcut instanceof IBeanDefinition) {
				advisorDef.getPropertyValues().add(POINTCUT, pointcut);
				parserContext.registerComponent(
						new AdvisorComponentDefinition(advisorBeanName, advisorDef, (IBeanDefinition) pointcut));
			}
	...
	private void parseAspect(Element aspectElement, ParserContext parserContext) {
		...
		try {
			...
			List<IBeanDefinition> beanDefinitions = new ArrayList<>();
	...
	private AspectComponentDefinition createAspectComponentDefinition(
			Element aspectElement, String aspectId, List<IBeanDefinition> beanDefs,
			List<BeanReference> beanRefs, ParserContext parserContext) {

		IBeanDefinition[] beanDefArray = beanDefs.toArray(new IBeanDefinition[0]);
	...
	private AbstractBeanDefinition parseAdvice(
			...
			List<IBeanDefinition> beanDefinitions, List<BeanReference> beanReferences) {
	...
	private AbstractBeanDefinition createAdviceDefinition(
		...
		if (pointcut instanceof IBeanDefinition) {
			...
			beanDefinitions.add((IBeanDefinition) pointcut);
		}
		else if (pointcut instanceof String) {
	 ...
	 * {@link ghost.framework.beans.factory.config.BeanDefinition} for the pointcut if  necessary
	...
	/**
	 * Creates a {@link IBeanDefinition} for the {@link AspectJExpressionPointcut} class using
	 * the supplied pointcut expression.
	 */
	protected AbstractBeanDefinition createPointcutDefinition(String expression) {
		RootBeanDefinition beanDefinition = new RootBeanDefinition(AspectJExpressionPointcut.class);
//		beanDefinition.setScope(IBeanDefinition.SCOPE_PROTOTYPE);
