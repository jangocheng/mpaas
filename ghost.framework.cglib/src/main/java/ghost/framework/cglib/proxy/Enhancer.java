...
package ghost.framework.cglib.proxy;

//import ghost.framework.asm.ClassVisitor;
//import ghost.framework.asm.Label;
//import ghost.framework.asm.Type;

import ghost.framework.cglib.core.AbstractClassGenerator;
import ghost.framework.cglib.core.KeyFactory;
import ghost.framework.cglib.core.ReflectUtils;
...
public class Enhancer extends AbstractClassGenerator {
	...
	/**
	 * {@link ghost.framework.cglib.core.AbstractClassGenerator.ClassLoaderData#generatedClasses} requires to keep cache key
	...
	private static final Type FACTORY =
			Type.getObjectType("ghost.framework.cglib.proxy.Factory");

	private static final Type ILLEGAL_STATE_EXCEPTION =
			Type.getObjectType("IllegalStateException");

	private static final Type ILLEGAL_ARGUMENT_EXCEPTION =
			Type.getObjectType("IllegalArgumentException");

	private static final Type THREAD_LOCAL =
			Type.getObjectType("ThreadLocal");

	private static final Type CALLBACK =
			Type.getObjectType("ghost.framework.cglib.proxy.Callback");
	...
	private static final Signature SET_THREAD_CALLBACKS =
			new Signature(SET_THREAD_CALLBACKS_NAME, null/*Type.VOID_TYPE*/, null/*new Type[]{CALLBACK_ARRAY}*/);

	private static final Signature SET_STATIC_CALLBACKS =
			new Signature(SET_STATIC_CALLBACKS_NAME, null/*Type.VOID_TYPE*/, null/*new Type[]{CALLBACK_ARRAY}*/);

	private static final Signature NEW_INSTANCE = null;//
			//new Signature("newInstance", Constants.TYPE_OBJECT, new Type[]{CALLBACK_ARRAY});

	private static final Signature MULTIARG_NEW_INSTANCE = null;//
...
//			});
	...
	private void preValidate() {
		if (callbackTypes == null) {
			callbackTypes = null;// CallbackInfo.determineTypes(callbacks, false);
	...
	private void validate() {
		...
		if (callbacks != null && callbackTypes != null) {
			...
			Type[] check = null;//CallbackInfo.determineTypes(callbacks);
			...
			callbackTypes = null;//CallbackInfo.determineTypes(callbacks);
		}
		if (interfaces != null) {
	...
	public void generateClass(ClassVisitor v) throws Exception {
		...
		ClassEmitter e = null;//new ClassEmitter(v);
		if (currentData == null) {
			e.begin_class(Constants.V1_8,
					Constants.ACC_PUBLIC,
					getClassName(),
					null/*Type.getType(sc)*/,
					(useFactory ?
							null/*TypeUtils.add(TypeUtils.getTypes(interfaces), FACTORY) */:
		...
		e.declare_field(Constants.ACC_PRIVATE, BOUND_FIELD, null/*Type.BOOLEAN_TYPE*/, null);
		e.declare_field(Constants.ACC_PUBLIC | Constants.ACC_STATIC, FACTORY_DATA_FIELD, null/*OBJECT_TYPE*/, null);
		if (!interceptDuringConstruction) {
			e.declare_field(Constants.ACC_PRIVATE, CONSTRUCTED_FIELD, null/*Type.BOOLEAN_TYPE*/, null);
		}
		e.declare_field(Constants.PRIVATE_FINAL_STATIC, THREAD_CALLBACKS_FIELD, null/*THREAD_LOCAL*/, null);
		e.declare_field(Constants.PRIVATE_FINAL_STATIC, STATIC_CALLBACKS_FIELD, null/*CALLBACK_ARRAY*/, null);
		if (serialVersionUID != null) {
			e.declare_field(Constants.PRIVATE_FINAL_STATIC, Constants.SUID_FIELD_NAME, null/*Type.LONG_TYPE*/, serialVersionUID);
		}

		for (int i = 0; i < callbackTypes.length; i++) {
			e.declare_field(Constants.ACC_PRIVATE, getCallbackField(i), null/*callbackTypes[i]*/, null);
		}
		// This is declared private to avoid "public field" pollution
		e.declare_field(Constants.ACC_PRIVATE | Constants.ACC_STATIC, CALLBACK_FILTER_FIELD, null/*OBJECT_TYPE*/, null);
