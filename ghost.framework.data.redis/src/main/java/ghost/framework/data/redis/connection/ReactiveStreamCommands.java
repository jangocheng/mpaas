...
public interface ReactiveStreamCommands {
	 ...
	 */
	class AcknowledgeCommand extends ReactiveRedisConnection.KeyCommand {
	 ...
	 * @see <a href="https://redis.io/commands/xack">Redis Documentation: XACK</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<AcknowledgeCommand, Long>> xAck(Publisher<AcknowledgeCommand> commands);
	 ...
	 * @see <a href="https://redis.io/commands/xadd">Redis Documentation: XADD</a>
	 */
	class AddStreamRecord extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<RecordId> xAdd(ByteBufferRecord record) {
		...
		return xAdd(Mono.just(AddStreamRecord.of(record))).next().map(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/xadd">Redis Documentation: XADD</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<AddStreamRecord, RecordId>> xAdd(Publisher<AddStreamRecord> commands);
	 ...
	 * @since 2.3
	 */
	default Flux<RecordId> xClaimJustId(ByteBuffer key, String group, String newOwner, RedisStreamCommands.XClaimOptions options) {

		return xClaimJustId(Mono.just(new XClaimCommand(key, group, newOwner, options))).next()
				.flatMapMany(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<XClaimCommand, Flux<RecordId>>> xClaimJustId(Publisher<XClaimCommand> commands);
	...
	default Flux<ByteBufferRecord> xClaim(ByteBuffer key, String group, String newOwner, Duration minIdleTime,
		...
		return xClaim(key, group, newOwner, RedisStreamCommands.XClaimOptions.minIdle(minIdleTime).ids(recordIds));
	}
	 ...
	 * @since 2.3
	 */
	default Flux<ByteBufferRecord> xClaim(ByteBuffer key, String group, String newOwner, RedisStreamCommands.XClaimOptions options) {
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<XClaimCommand, Flux<ByteBufferRecord>>> xClaim(Publisher<XClaimCommand> commands);
	 ...
	 * @since 2.3
	 */
	class XClaimCommand extends ReactiveRedisConnection.KeyCommand {

		private final String groupName;
		private final String newOwner;
		private final RedisStreamCommands.XClaimOptions options;

		private XClaimCommand(@Nullable ByteBuffer key, String groupName, String newOwner, RedisStreamCommands.XClaimOptions options) {
	 ...
	 */
	class DeleteCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> xDel(ByteBuffer key, String... recordIds) {
		...
		return xDel(Mono.just(DeleteCommand.stream(key).records(recordIds))).next().map(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/xdel">Redis Documentation: XDEL</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<DeleteCommand, Long>> xDel(Publisher<DeleteCommand> commands);
	...
	default Mono<Long> xLen(ByteBuffer key) {
		...
		return xLen(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}

	/**
	 * Get the size of the stream stored at {@link ReactiveRedisConnection.KeyCommand#getKey()}
	 *
	 * @param commands must not be {@literal null}.
	 * @return {@link Flux} emitting the length of the stream per {@link ReactiveRedisConnection.KeyCommand}.
	 * @see <a href="https://redis.io/commands/xlen">Redis Documentation: XLEN</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> xLen(Publisher<ReactiveRedisConnection.KeyCommand> commands);
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<PendingRecordsCommand, PendingMessagesSummary>> xPendingSummary(
	...
	/**
	 * Obtain detailed information about pending {@link PendingMessage messages} applying given {@link RedisStreamCommands.XPendingOptions
	 * options}.
	 *
	 * @param commands must not be {@literal null}.
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<PendingRecordsCommand, PendingMessages>> xPending(Publisher<PendingRecordsCommand> commands);
	 ...
	 * @since 2.3
	 */
	class PendingRecordsCommand extends ReactiveRedisConnection.KeyCommand {
		 ...
		 *
		 * @return new instance of {@link RedisStreamCommands.XPendingOptions}.
		 */
	 ...
	 */
	class RangeCommand extends ReactiveRedisConnection.KeyCommand {

		private final Range<String> range;
		private final RedisZSetCommands.Limit limit;

		/**
		 * Creates a new {@link RangeCommand} given a {@code key}, {@link Range}, and {@link RedisZSetCommands.Limit}.
		 *
		 * @param key must not be {@literal null}.
		...
		public static RangeCommand stream(ByteBuffer key) {
			return new RangeCommand(key, Range.unbounded(), RedisZSetCommands.Limit.unlimited());
		}
		/**
		...
		public RangeCommand limit(int count) {
			return new RangeCommand(getKey(), range, RedisZSetCommands.Limit.unlimited().count(count));
		}
		/**
		 * Applies a {@code Limit}. Constructs a new command instance with all previously configured properties.
		 *
		...
		/**
		 * @return the {@link RedisZSetCommands.Limit}.
		 */
	...
	/**
	 * Read records from a stream within a specific {@link Range} applying a {@link RedisZSetCommands.Limit}.
	 *
	 * @param key the stream key.
	 * @param range must not be {@literal null}.
	 * @param limit must not be {@literal null}.
	 ...
	 * @see <a href="https://redis.io/commands/xrange">Redis Documentation: XRANGE</a>
	 */
	default Flux<ByteBufferRecord> xRange(ByteBuffer key, Range<String> range, RedisZSetCommands.Limit limit) {
	 ...
	 * @see <a href="https://redis.io/commands/xrange">Redis Documentation: XRANGE</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<RangeCommand, Flux<ByteBufferRecord>>> xRange(Publisher<RangeCommand> commands);
	 ...
	 */
	Flux<ReactiveRedisConnection.CommandResponse<ReadCommand, Flux<ByteBufferRecord>>> read(Publisher<ReadCommand> commands);

	/**
	 * @author Christoph Strobl
	 * @since 2.3
	 */
	class XInfoCommand extends ReactiveRedisConnection.KeyCommand {
	 ...
	 *
	 * @param key the {@literal key} the stream is stored at.
	 * @return a {@link Mono} emitting {@link StreamInfo.XInfoStream} when ready.
	 * @since 2.3
	 */
	default Mono<StreamInfo.XInfoStream> xInfo(ByteBuffer key) {
		return xInfo(Mono.just(XInfoCommand.of(key))).next().map(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<XInfoCommand, StreamInfo.XInfoStream>> xInfo(Publisher<XInfoCommand> commands);
	 ...
	 * @since 2.3
	 */
	default Flux<StreamInfo.XInfoGroup> xInfoGroups(ByteBuffer key) {
		return xInfoGroups(Mono.just(XInfoCommand.of(key))).next().flatMapMany(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<XInfoCommand, Flux<StreamInfo.XInfoGroup>>> xInfoGroups(Publisher<XInfoCommand> commands);
	 ...
	 * @since 2.3
	 */
	default Flux<StreamInfo.XInfoConsumer> xInfoConsumers(ByteBuffer key, String groupName) {
	 ...
	 * @since 2.3
	 */
	Flux<ReactiveRedisConnection.CommandResponse<XInfoCommand, Flux<StreamInfo.XInfoConsumer>>> xInfoConsumers(Publisher<XInfoCommand> commands);

	public class GroupCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<String> xGroup(GroupCommand command) {
		return xGroup(Mono.just(command)).next().map(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	/**
	 * Execute the given {@link GroupCommand} to {@literal create, destroy,... } groups.
	 *
	 ...
	 */
	Flux<ReactiveRedisConnection.CommandResponse<GroupCommand, String>> xGroup(Publisher<GroupCommand> commands);
	...
	/**
	 * Read records from a stream within a specific {@link Range} applying a {@link RedisZSetCommands.Limit} in reverse order.
	 *
	 * @param key the stream key.
	 * @param range must not be {@literal null}.
	 * @param limit must not be {@literal null}.
	 ...
	 * @see <a href="https://redis.io/commands/xrevrange">Redis Documentation: XREVRANGE</a>
	 */
	default Flux<ByteBufferRecord> xRevRange(ByteBuffer key, Range<String> range, RedisZSetCommands.Limit limit) {
		Assert.notNull(key, "Key must not be null!");
		Assert.notNull(range, "Range must not be null!");
		Assert.notNull(limit, "Limit must not be null!");
	 ...
	 * @see <a href="https://redis.io/commands/xrevrange">Redis Documentation: XREVRANGE</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<RangeCommand, Flux<ByteBufferRecord>>> xRevRange(Publisher<RangeCommand> commands);
	 ...
	 */
	class TrimCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> xTrim(ByteBuffer key, long count) {
		...
		return xTrim(Mono.just(TrimCommand.stream(key).to(count))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/xtrim">Redis Documentation: XTRIM</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> xTrim(Publisher<TrimCommand> commands);
}
