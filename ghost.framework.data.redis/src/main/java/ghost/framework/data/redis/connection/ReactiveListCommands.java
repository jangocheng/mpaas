...
public interface ReactiveListCommands {
	 ...
	 */
	class PushCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> rPush(ByteBuffer key, List<ByteBuffer> values) {
		...
		return push(Mono.just(PushCommand.right().values(values).to(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	...
	default Mono<Long> rPushX(ByteBuffer key, ByteBuffer value) {
		...
		return push(Mono.just(PushCommand.right().value(value).to(key).ifExists())).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	...
	default Mono<Long> lPush(ByteBuffer key, List<ByteBuffer> values) {
		...
		Assert.notNull(key, "Key must not be null!");
		Assert.notNull(values, "Values must not be null!");
		return push(Mono.just(PushCommand.left().values(values).to(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	/**
	...
	default Mono<Long> lPushX(ByteBuffer key, ByteBuffer value) {
		...
		return push(Mono.just(PushCommand.left().value(value).to(key).ifExists())).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/rpush">Redis Documentation: RPUSH</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<PushCommand, Long>> push(Publisher<PushCommand> commands);
	/**
	...
	default Mono<Long> lLen(ByteBuffer key) {
		...
		return lLen(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}

	/**
	 * Get the size of list stored at {@link ReactiveRedisConnection.KeyCommand#getKey()}
	 *
	 * @param commands must not be {@literal null}.
	 * @return
	 * @see <a href="https://redis.io/commands/llen">Redis Documentation: LLEN</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> lLen(Publisher<ReactiveRedisConnection.KeyCommand> commands);
	...
	default Flux<ByteBuffer> lRange(ByteBuffer key, long start, long end) {
		...
		return lRange(Mono.just(ReactiveRedisConnection.RangeCommand.key(key).fromIndex(start).toIndex(end))).flatMap(ReactiveRedisConnection.CommandResponse::getOutput);
	}

	/**
	 * Get elements in {@link ReactiveRedisConnection.RangeCommand#getRange()} from list at {@link ReactiveRedisConnection.RangeCommand#getKey()}
	 *
	 * @param commands must not be {@literal null}.
	 * @return
	 * @see <a href="https://redis.io/commands/lrange">Redis Documentation: LRANGE</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<ReactiveRedisConnection.RangeCommand, Flux<ByteBuffer>>> lRange(Publisher<ReactiveRedisConnection.RangeCommand> commands);
	/**
	...
	default Mono<Boolean> lTrim(ByteBuffer key, long start, long end) {
		...
		return lTrim(Mono.just(ReactiveRedisConnection.RangeCommand.key(key).fromIndex(start).toIndex(end))) //
				.next() //
				.map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}

	/**
	 * Trim list at {@link ReactiveRedisConnection.RangeCommand#getKey()} to elements within {@link ReactiveRedisConnection.RangeCommand#getRange()}.
	 *
	 * @param commands must not be {@literal null}.
	 * @return
	 * @see <a href="https://redis.io/commands/ltrim">Redis Documentation: LTRIM</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<ReactiveRedisConnection.RangeCommand>> lTrim(Publisher<ReactiveRedisConnection.RangeCommand> commands);
	 ...
	 */
	class LIndexCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<ByteBuffer> lIndex(ByteBuffer key, long index) {
		...
		return lIndex(Mono.just(LIndexCommand.elementAt(index).from(key))).next().map(ReactiveRedisConnection.ByteBufferResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/lindex">Redis Documentation: LINDEX</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<LIndexCommand>> lIndex(Publisher<LIndexCommand> commands);
	 ...
	 */
	class LInsertCommand extends ReactiveRedisConnection.KeyCommand {
		...
		private LInsertCommand(@Nullable ByteBuffer key, @Nullable RedisListCommands.Position position, @Nullable ByteBuffer pivot,
							   ByteBuffer value) {
		...
		public LInsertCommand before(ByteBuffer pivot) {
			...
			return new LInsertCommand(getKey(), RedisListCommands.Position.BEFORE, pivot, value);
		}
		...
		public LInsertCommand after(ByteBuffer pivot) {
			...
			return new LInsertCommand(getKey(), RedisListCommands.Position.AFTER, pivot, value);
		}
	...
	/**
	 * Insert {@literal value} {@link RedisListCommands.Position#BEFORE} or {@link RedisListCommands.Position#AFTER} existing {@literal pivot} for
	 * {@literal key}.
	 *
	 * @param key must not be {@literal null}.
	 ...
	 * @see <a href="https://redis.io/commands/linsert">Redis Documentation: LINSERT</a>
	 */
	default Mono<Long> lInsert(ByteBuffer key, RedisListCommands.Position position, ByteBuffer pivot, ByteBuffer value) {
		...
		command = RedisListCommands.Position.BEFORE.equals(position) ? command.before(pivot) : command.after(pivot);
		command = command.forKey(key);
		return lInsert(Mono.just(command)).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}

	/**
	 * Insert {@link LInsertCommand#getValue()} {@link RedisListCommands.Position#BEFORE} or {@link RedisListCommands.Position#AFTER} existing
	 ...
	 * @see <a href="https://redis.io/commands/linsert">Redis Documentation: LINSERT</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<LInsertCommand, Long>> lInsert(Publisher<LInsertCommand> commands);
	 ...
	 */
	class LSetCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Boolean> lSet(ByteBuffer key, long index, ByteBuffer value) {
		...
		return lSet(Mono.just(LSetCommand.elementAt(index).to(value).forKey(key))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/lset">Redis Documentation: LSET</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<LSetCommand>> lSet(Publisher<LSetCommand> commands);
	 ...
	 */
	class LRemCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> lRem(ByteBuffer key, ByteBuffer value) {
		...
		return lRem(Mono.just(LRemCommand.all().occurrencesOf(value).from(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/lrem">Redis Documentation: LREM</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<LRemCommand, Long>> lRem(Publisher<LRemCommand> commands);
	/**
	 ...
	 */
	class PopCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<ByteBuffer> lPop(ByteBuffer key) {
		...
		return pop(Mono.just(PopCommand.left().from(key))).next().map(ReactiveRedisConnection.ByteBufferResponse::getOutput);
	}
	...
	default Mono<ByteBuffer> rPop(ByteBuffer key) {
		...
		return pop(Mono.just(PopCommand.right().from(key))).next().map(ReactiveRedisConnection.ByteBufferResponse::getOutput);
	}

	/**
	 * Removes and returns last element in list stored at {@link ReactiveRedisConnection.KeyCommand#getKey()}
	 *
	 * @param commands must not be {@literal null}.
	 * @return
	 * @see <a href="https://redis.io/commands/lpop">Redis Documentation: LPOP</a>
	 * @see <a href="https://redis.io/commands/rpop">Redis Documentation: RPOP</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<PopCommand>> pop(Publisher<PopCommand> commands);
	/**
	 * @author Christoph Strobl
	 * @see <a href="https://redis.io/commands/blpop">Redis Documentation: BLPOP</a>
	 * @see <a href="https://redis.io/commands/brpop">Redis Documentation: BRPOP</a>
	 */
	class BPopCommand implements ReactiveRedisConnection.Command {
	 ...
	 * @author Christoph Strobl
	 */
	class PopResponse extends ReactiveRedisConnection.CommandResponse<BPopCommand, PopResult> {
	 ...
	 */
	class RPopLPushCommand extends ReactiveRedisConnection.KeyCommand {
	 ...
	 * @see <a href="https://redis.io/commands/rpoplpush">Redis Documentation: RPOPLPUSH</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<RPopLPushCommand>> rPopLPush(Publisher<RPopLPushCommand> commands);
	 ...
	 */
	class BRPopLPushCommand extends ReactiveRedisConnection.KeyCommand {
	 ...
	 * @see <a href="https://redis.io/commands/brpoplpush">Redis Documentation: BRPOPLPUSH</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<BRPopLPushCommand>> bRPopLPush(Publisher<BRPopLPushCommand> commands);
}
