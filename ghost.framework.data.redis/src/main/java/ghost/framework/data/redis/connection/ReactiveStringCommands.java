...
public interface ReactiveStringCommands {
	 ...
	 */
	class SetCommand extends ReactiveRedisConnection.KeyCommand {
		...
		private SetCommand(ByteBuffer key, @Nullable ByteBuffer value, @Nullable Expiration expiration,
				@Nullable RedisStringCommands.SetOption option) {
		...
		/**
		 * Applies {@link RedisStringCommands.SetOption}. Constructs a new command instance with all previously configured properties.
		 *
		 * @param option must not be {@literal null}.
		 * @return a new {@link SetCommand} with {@link RedisStringCommands.SetOption} applied.
		 */
	...
	default Mono<Boolean> set(ByteBuffer key, ByteBuffer value) {
		...
		return set(Mono.just(SetCommand.set(key).value(value))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/set">Redis Documentation: SET</a>
	 */
	default Mono<Boolean> set(ByteBuffer key, ByteBuffer value, Expiration expiration, RedisStringCommands.SetOption option) {

		Assert.notNull(key, "Key must not be null!");
		Assert.notNull(value, "Value must not be null!");

		return set(Mono.just(SetCommand.set(key).value(value).withSetOption(option).expiring(expiration))).next()
				.map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @return {@link Flux} of {@link ReactiveRedisConnection.BooleanResponse} holding the {@link SetCommand} along with the command result.
	 * @see <a href="https://redis.io/commands/set">Redis Documentation: SET</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<SetCommand>> set(Publisher<SetCommand> commands);
	...
	default Mono<ByteBuffer> get(ByteBuffer key) {
		...
		return get(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).next().filter(ReactiveRedisConnection.CommandResponse::isPresent)
	 ...
	 * @return {@link Flux} of {@link ReactiveRedisConnection.ByteBufferResponse} holding the {@literal key} to get along with the value
	 *         retrieved.
	 * @see <a href="https://redis.io/commands/get">Redis Documentation: GET</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<ReactiveRedisConnection.KeyCommand>> get(Publisher<ReactiveRedisConnection.KeyCommand> keys);
	...
	default Mono<ByteBuffer> getSet(ByteBuffer key, ByteBuffer value) {
		...
		return getSet(Mono.just(SetCommand.set(key).value(value))).next().filter(ReactiveRedisConnection.CommandResponse::isPresent)
				.map(ReactiveRedisConnection.ByteBufferResponse::getOutput);
	}
	 ...
	 *
	 * @param commands must not be {@literal null}.
	 * @return {@link Flux} of {@link ReactiveRedisConnection.ByteBufferResponse} holding the {@link SetCommand} along with the previously
	 *         existing value.
	 * @see <a href="https://redis.io/commands/getset">Redis Documentation: GETSET</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<SetCommand>> getSet(Publisher<SetCommand> commands);
	...
	default Mono<List<ByteBuffer>> mGet(List<ByteBuffer> keys) {
		...
		return mGet(Mono.just(keys)).next().map(ReactiveRedisConnection.MultiValueResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/mget">Redis Documentation: MGET</a>
	 */
	Flux<ReactiveRedisConnection.MultiValueResponse<List<ByteBuffer>, ByteBuffer>> mGet(Publisher<List<ByteBuffer>> keysets);
	...
	default Mono<Boolean> setNX(ByteBuffer key, ByteBuffer value) {
		...
		return setNX(Mono.just(SetCommand.set(key).value(value))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	/**
	 ...
	 * @see <a href="https://redis.io/commands/setnx">Redis Documentation: SETNX</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<SetCommand>> setNX(Publisher<SetCommand> values);
	 ...
	 * @see <a href="https://redis.io/commands/setex">Redis Documentation: SETEX</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<SetCommand>> setEX(Publisher<SetCommand> commands);
	 ...
	 * @see <a href="https://redis.io/commands/psetex">Redis Documentation: PSETEX</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<SetCommand>> pSetEX(Publisher<SetCommand> commands);
	 ...
	 */
	class MSetCommand implements ReactiveRedisConnection.Command {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.redis.connection.ReactiveRedisConnection.Command#getKey()
		 */
		@Override
		@Nullable
	...
	default Mono<Boolean> mSet(Map<ByteBuffer, ByteBuffer> keyValuePairs) {
		...
		return mSet(Mono.just(MSetCommand.mset(keyValuePairs))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/mset">Redis Documentation: MSET</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<MSetCommand>> mSet(Publisher<MSetCommand> commands);
	...
	default Mono<Boolean> mSetNX(Map<ByteBuffer, ByteBuffer> keyValuePairs) {
		...
		return mSetNX(Mono.just(MSetCommand.mset(keyValuePairs))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/msetnx">Redis Documentation: MSETNX</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<MSetCommand>> mSetNX(Publisher<MSetCommand> source);
	 ...
	 */
	class AppendCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> append(ByteBuffer key, ByteBuffer value) {
		...
		return append(Mono.just(AppendCommand.key(key).append(value))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/append">Redis Documentation: APPEND</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<AppendCommand, Long>> append(Publisher<AppendCommand> commands);
	...
	default Mono<ByteBuffer> getRange(ByteBuffer key, long start, long end) {
		...
		return getRange(Mono.just(ReactiveRedisConnection.RangeCommand.key(key).fromIndex(start).toIndex(end))) //
				.next() //
				.map(ReactiveRedisConnection.ByteBufferResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/getrange">Redis Documentation: GETRANGE</a>
	 */
	Flux<ReactiveRedisConnection.ByteBufferResponse<ReactiveRedisConnection.RangeCommand>> getRange(Publisher<ReactiveRedisConnection.RangeCommand> commands);
	 ...
	 */
	class SetRangeCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> setRange(ByteBuffer key, ByteBuffer value, long offset) {
		...
		return setRange(Mono.just(SetRangeCommand.overwrite(key).withValue(value).atPosition(offset))).next()
				.map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/setrange">Redis Documentation: SETRANGE</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<SetRangeCommand, Long>> setRange(Publisher<SetRangeCommand> commands);
	 ...
	 */
	class GetBitCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Boolean> getBit(ByteBuffer key, long offset) {
		...
		return getBit(Mono.just(GetBitCommand.bit(key).atOffset(offset))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/getbit">Redis Documentation: GETBIT</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<GetBitCommand>> getBit(Publisher<GetBitCommand> commands);
	 ...
	 */
	class SetBitCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Boolean> setBit(ByteBuffer key, long offset, boolean value) {
		...
		return setBit(Mono.just(SetBitCommand.bit(key).atOffset(offset).to(value))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/setbit">Redis Documentation: SETBIT</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<SetBitCommand>> setBit(Publisher<SetBitCommand> commands);
	 ...
	 */
	class BitCountCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> bitCount(ByteBuffer key) {
		...
		return bitCount(Mono.just(BitCountCommand.bitCount(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/bitcount">Redis Documentation: BITCOUNT</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<BitCountCommand, Long>> bitCount(Publisher<BitCountCommand> commands);
	/**
	 ...
	 * @since 2.1
	 */
	class BitFieldCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<List<Long>> bitField(ByteBuffer key, BitFieldSubCommands subCommands) {
		...
		return bitField(Mono.just(BitFieldCommand.bitField(key).commands(subCommands))).map(ReactiveRedisConnection.CommandResponse::getOutput)
				.next();
	}
	 ...
	 * @since 2.1
	 */
	Flux<ReactiveRedisConnection.MultiValueResponse<BitFieldCommand, Long>> bitField(Publisher<BitFieldCommand> commands);
	...
	class BitOpCommand {
		...
		/**
		 * Creates a new {@link BitOpCommand} given a {@link RedisStringCommands.BitOperation}.
		 *
		 * @param bitOp must not be {@literal null}.
		 * @return a new {@link BitCountCommand} for a {@link RedisStringCommands.BitOperation}.
		 */
	 ...
	 * @see <a href="https://redis.io/commands/bitop">Redis Documentation: BITOP</a>
	 */
	default Mono<Long> bitOp(Collection<ByteBuffer> keys, RedisStringCommands.BitOperation bitOp, ByteBuffer destination) {
	 ...
	 * @see <a href="https://redis.io/commands/bitop">Redis Documentation: BITOP</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<BitOpCommand, Long>> bitOp(Publisher<BitOpCommand> commands);

	/**
	 * @author Christoph Strobl
	 * @since 2.1
	 */
	class BitPosCommand extends ReactiveRedisConnection.KeyCommand {
	 ...
	 * @since 2.1
	 */
	Flux<ReactiveRedisConnection.NumericResponse<BitPosCommand, Long>> bitPos(Publisher<BitPosCommand> commands);
	...
	default Mono<Long> strLen(ByteBuffer key) {
		...
		return strLen(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/strlen">Redis Documentation: STRLEN</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> strLen(Publisher<ReactiveRedisConnection.KeyCommand> keys);
}
