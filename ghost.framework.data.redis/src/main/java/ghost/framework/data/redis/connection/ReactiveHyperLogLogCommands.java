...
public interface ReactiveHyperLogLogCommands {
	 ...
	 */
	class PfAddCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> pfAdd(ByteBuffer key, Collection<ByteBuffer> values) {
		...
		return pfAdd(Mono.just(PfAddCommand.values(values).to(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/pfadd">Redis Documentation: PFADD</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<PfAddCommand, Long>> pfAdd(Publisher<PfAddCommand> commands);
	 ...
	 */
	class PfCountCommand implements ReactiveRedisConnection.Command {
	...
	default Mono<Long> pfCount(Collection<ByteBuffer> keys) {
		...
		return pfCount(Mono.just(PfCountCommand.valuesIn(keys))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/pfcount">Redis Documentation: PFCOUNT</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<PfCountCommand, Long>> pfCount(Publisher<PfCountCommand> commands);
	 ...
	 */
	class PfMergeCommand extends ReactiveRedisConnection.KeyCommand {
	 ...
	 * @see <a href="https://redis.io/commands/pfmerge">Redis Documentation: PFMERGE</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<PfMergeCommand>> pfMerge(Publisher<PfMergeCommand> commands);
}
