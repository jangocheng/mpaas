...
import ghost.framework.context.bean.ManagedMap;
...
import ghost.framework.data.redis.listener.adapter.MessageListenerAdapter;
...
class RedisListenerContainerParser extends AbstractSimpleBeanDefinitionParser {
	...
	protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
		...
		if (!listDefs.isEmpty()) {
			ManagedMap<IBeanDefinition, Collection<? extends IBeanDefinition>> listeners = new ManagedMap<>(listDefs.size());
			for (Element listElement : listDefs) {
				Object[] listenerDefinition = parseListener(listElement);
				listeners.put((IBeanDefinition) listenerDefinition[0],
						(Collection<? extends IBeanDefinition>) listenerDefinition[1]);
			}
