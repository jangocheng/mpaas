...
import ghost.framework.context.utils.comparator.NullSafeComparator;
...
import ghost.framework.data.commons.mapping.model.EntityInstantiator;
import ghost.framework.data.commons.mapping.model.EntityInstantiators;
import ghost.framework.data.commons.mapping.model.PersistentEntityParameterValueProvider;
import ghost.framework.data.commons.mapping.model.PropertyValueProvider;
import ghost.framework.data.commons.util.ClassTypeInformation;
import ghost.framework.data.commons.util.TypeInformation;
import ghost.framework.data.redis.core.PartialUpdate;
...
public class MappingRedisConverter implements RedisConverter {
	...
	public MappingRedisConverter(@Nullable RedisMappingContext mappingContext, @Nullable IndexResolver indexResolver,
		...
		this.conversionService = null;//new DefaultConversionService();
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.convert.EntityReader#read(java.lang.Class, java.lang.Object)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.convert.EntityWriter#write(java.lang.Object, java.lang.Object)
	 */
	@Override
	...
	public void write(Object source, RedisData sink) {
		...
		if (entity == null) {
			...
			typeMapper.writeType(ClassUtils.getUserClass(source), sink.getBucket().getPath());
			sink.getBucket().put("_raw",( byte[]) conversionService.convert(source, byte[].class));
			return;
		}
		...
		if (identifier != null) {
			sink.setId((String) getConversionService().convert(identifier, String.class));
		}
	 ...
	 * @param path
	 */
	private void writePartialPropertyUpdate(PartialUpdate<?> update, PartialUpdate.PropertyUpdate pUpdate, RedisData sink,
	...
	private void writeInternal(String keyspace, String path, @Nullable Object value, TypeInformation<?> typeHint,
		...
		if (customConversions.hasCustomWriteTarget(value.getClass())) {
			...
			if (!StringUtils.hasText(path) && customConversions.getCustomWriteTarget(value.getClass()).equals(byte[].class)) {
				sink.getBucket().put(StringUtils.hasText(path) ? path : "_raw", (byte[])conversionService.convert(value, byte[].class));
			} else {
	...
	private String mapMapKey(Object key) {
		...
		if (conversionService.canConvert(key.getClass(), byte[].class)) {
			return new String((byte[])conversionService.convert(key, byte[].class));
		}
	...
	public byte[] toBytes(Object source) {
		...
		return (byte[])conversionService.convert(source, byte[].class);
	}
	...
	public <T> T fromBytes(byte[] source, Class<T> type) {
		return (T)conversionService.convert(source, type);
	}
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.core.convert.RedisConverter#getIndexResolver()
	 */
	@Nullable
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.convert.EntityConverter#getConversionService()
	 */
//	@Override
	...
	private class ConverterAwareParameterValueProvider implements PropertyValueProvider<RedisPersistentProperty> {
		...
		ConverterAwareParameterValueProvider(String path, RedisData source, ConverterContainer conversionService) {
		...
		public <T> T getPropertyValue(RedisPersistentProperty property) {
			...
			return (T) converterContainer.convert(value, property.getType());
		}
	}
