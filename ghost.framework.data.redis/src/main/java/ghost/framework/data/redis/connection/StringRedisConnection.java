...
import ghost.framework.data.redis.core.StringRedisTemplate;
...
public interface StringRedisConnection extends RedisConnection {
	...
	default Long bitPos(String key, boolean bit) {
		return bitPos(key, bit, ghost.framework.data.domain.Range.unbounded());
	}

	/**
	 * Return the position of the first bit set to given {@code bit} in a string.
	 * {@link ghost.framework.data.domain.Range} start and end can contain negative values in order to index
	 ...
	 */
	@Nullable
	Long bitPos(String key, boolean bit, ghost.framework.data.domain.Range<Long> range);
	 ...
	 * @since 1.6
	 * @see <a href="https://redis.io/commands/zrangebylex">Redis Documentation: ZRANGEBYLEX</a>
	 * @see RedisZSetCommands#zRangeByLex(byte[], Range, RedisZSetCommands.Limit)
	 */
	Set<String> zRangeByLex(String key, Range range, RedisZSetCommands.Limit limit);
	...
	default RecordId xAdd(StringRecord record) {
		return xAdd(record, RedisStreamCommands.XAddOptions.none());
	}
	 ...
	 *
	 * @param record must not be {@literal null}.
	 * @param options must not be {@literal null}, use {@link RedisStreamCommands.XAddOptions#none()} instead.
	 * @return the record Id. {@literal null} when used in pipeline / transaction.
	 * @since 2.3
	 */
	@Nullable
	RecordId xAdd(StringRecord record, RedisStreamCommands.XAddOptions options);
	 ...
	 * @since 2.3
	 */
	List<RecordId> xClaimJustId(String key, String group, String newOwner, RedisStreamCommands.XClaimOptions options);
	 ...
	 * @since 2.3
	 */
	List<StringRecord> xClaim(String key, String group, String newOwner, RedisStreamCommands.XClaimOptions options);
	 ...
	 */
	@Nullable
	StreamInfo.XInfoConsumers xInfoConsumers(String key, String groupName);
	 ...
	 * {@link ghost.framework.data.domain.Range} within a {@literal consumer group}.
	 *
	 * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
	 * @param groupName the name of the {@literal consumer group}. Must not be {@literal null}.
	 ...
	 */
	@Nullable
	PendingMessages xPending(String key, String groupName, ghost.framework.data.domain.Range<String> range,
                             Long count);
	 ...
	 */
	@Nullable
	PendingMessages xPending(String key, String groupName, RedisStreamCommands.XPendingOptions options);
	 ...
	 */
	@Nullable
	default List<StringRecord> xRange(String key, ghost.framework.data.domain.Range<String> range) {
	 ...
	 */
	@Nullable
	List<StringRecord> xRange(String key, ghost.framework.data.domain.Range<String> range, RedisZSetCommands.Limit limit);
	 ...
	 */
	@Nullable
	default List<StringRecord> xRevRange(String key, ghost.framework.data.domain.Range<String> range) {
	 ...
	 */
	@Nullable
	List<StringRecord> xRevRange(String key, ghost.framework.data.domain.Range<String> range, RedisZSetCommands.Limit limit);
