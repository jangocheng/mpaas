 ...
 */
package ghost.framework.data.redis.connection.stream;
 ...
 * @since 2.2
 */
public interface MapRecord<S, K, V> extends Record<S, Map<K, V>>, Iterable<Entry<K, V>> {
	 ...
	 */
	 static <S, K, V> MapRecord<S, K, V> create(S stream, Map<K, V> map) {
		...
		return new StreamRecords.MapBackedRecord<>(stream, RecordId.autoGenerate(), map);
	}

	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands.Record#withId(ghost.framework.data.redis.connection.RedisStreamCommands.RecordId)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.redis.connection.RedisStreamCommands.Record#withStreamKey(java.lang.Object)
	 */
	@Override
