...
import ghost.framework.thread.task.ThreadPoolTaskExecutor;
...
public class ClusterCommandExecutor implements AutoCloseable {
	...
	private <T> MultiNodeResult<T> collectResults(Map<NodeExecution, Future<NodeResult<T>>> futures) {
		...
		while (!done) {
			...
			for (Entry<NodeExecution, Future<NodeResult<T>>> entry : futures.entrySet()) {
	...
	public void close() throws Exception {
		...
		if (resourceProvider instanceof AutoCloseable) {
			((AutoCloseable) resourceProvider).close();
		}
	}
