...
import ghost.framework.data.commons.annotation.KeySpace;
...
import ghost.framework.data.commons.mapping.context.KeyValueMappingContext;
...
import ghost.framework.data.redis.core.TimeToLive;
import ghost.framework.data.redis.core.TimeToLiveAccessor;
...
import ghost.framework.data.redis.core.convert.RedisCustomConversions;
...
public class RedisMappingContext extends KeyValueMappingContext<RedisPersistentEntity<?>, RedisPersistentProperty> {
	...
	static class ConfigAwareKeySpaceResolver implements KeySpaceResolver {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.keyvalue.core.mapping.KeySpaceResolver#resolveKeySpace(java.lang.Class)
		 */
		@Override
	...
	enum ClassNameKeySpaceResolver implements KeySpaceResolver {
		...
		public String resolveKeySpace(Class<?> type) {
			...
			Assert.notNull(type, "Type must not be null!");
			return null;//ClassUtils.getUserClass(type).getName();
		}
	}
	...
	static class ConfigAwareTimeToLiveAccessor implements TimeToLiveAccessor {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.redis.core.TimeToLiveResolver#isExpiringEntity(java.lang.Class)
		 */
		@Override
		...
		private PersistentProperty<?> resolveTtlProperty(Class<?> type) {
			...
			if (keyspaceConfig.hasSettingsFor(type)) {
				...
				KeyspaceConfiguration.KeyspaceSettings settings = keyspaceConfig.getKeyspaceSettings(type);
