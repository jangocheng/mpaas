...
public interface ReactiveHashCommands {
	...
	/**
	 * {@literal HSET} {@link ReactiveRedisConnection.Command}.
	 *
	 * @author Christoph Strobl
	 * @see <a href="https://redis.io/commands/hset">Redis Documentation: HSET</a>
	 */
	class HSetCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Boolean> hSet(ByteBuffer key, ByteBuffer field, ByteBuffer value) {
		...
		return hSet(Mono.just(HSetCommand.value(value).ofField(field).forKey(key))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/hset">Redis Documentation: HSET</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<HSetCommand>> hSet(Publisher<HSetCommand> commands);
	/**
	 * {@literal HGET} {@link ReactiveRedisConnection.Command}.
	 *
	 * @author Christoph Strobl
	 * @see <a href="https://redis.io/commands/hget">Redis Documentation: HGET</a>
	 */
	class HGetCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<List<ByteBuffer>> hMGet(ByteBuffer key, Collection<ByteBuffer> fields) {
		...
		return hMGet(Mono.just(HGetCommand.fields(fields).from(key))).next().map(ReactiveRedisConnection.MultiValueResponse::getOutput);
	}
	/**
	 * Get values for given {@literal fields} from hash at {@literal key}.
	 *
	 ...
	 * @see <a href="https://redis.io/commands/hmget">Redis Documentation: HMGET</a>
	 */
	Flux<ReactiveRedisConnection.MultiValueResponse<HGetCommand, ByteBuffer>> hMGet(Publisher<HGetCommand> commands);

	/**
	 * {@literal HEXISTS} {@link ReactiveRedisConnection.Command}.
	 *
	 * @author Christoph Strobl
	 * @see <a href="https://redis.io/commands/hexists">Redis Documentation: HEXISTS</a>
	 */
	class HExistsCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Boolean> hExists(ByteBuffer key, ByteBuffer field) {
		...
		return hExists(Mono.just(HExistsCommand.field(field).in(key))).next().map(ReactiveRedisConnection.BooleanResponse::getOutput);
	}
	/**
	 * Determine if given hash {@literal field} exists.
	 *
	 ...
	 * @see <a href="https://redis.io/commands/hexists">Redis Documentation: HEXISTS</a>
	 */
	Flux<ReactiveRedisConnection.BooleanResponse<HExistsCommand>> hExists(Publisher<HExistsCommand> commands);

	/**
	 * @author Christoph Strobl
	 * @see <a href="https://redis.io/commands/hdel">Redis Documentation: HDEL</a>
	 */
	class HDelCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> hDel(ByteBuffer key, Collection<ByteBuffer> fields) {
		...
		return hDel(Mono.just(HDelCommand.fields(fields).from(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/hdel">Redis Documentation: HDEL</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<HDelCommand, Long>> hDel(Publisher<HDelCommand> commands);
	...
	default Mono<Long> hLen(ByteBuffer key) {
		...
		return hLen(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/hlen">Redis Documentation: HLEN</a>
	 */
	Flux<ReactiveRedisConnection.NumericResponse<ReactiveRedisConnection.KeyCommand, Long>> hLen(Publisher<ReactiveRedisConnection.KeyCommand> commands);
	...
	default Flux<ByteBuffer> hKeys(ByteBuffer key) {
		...
		return hKeys(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).flatMap(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/hkeys">Redis Documentation: HKEYS</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<ReactiveRedisConnection.KeyCommand, Flux<ByteBuffer>>> hKeys(Publisher<ReactiveRedisConnection.KeyCommand> commands);
	...
	default Flux<ByteBuffer> hVals(ByteBuffer key) {
		...
		return hVals(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).flatMap(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/hvals">Redis Documentation: HVALS</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<ReactiveRedisConnection.KeyCommand, Flux<ByteBuffer>>> hVals(Publisher<ReactiveRedisConnection.KeyCommand> commands);
	...
	default Flux<Map.Entry<ByteBuffer, ByteBuffer>> hGetAll(ByteBuffer key) {
		...
		return hGetAll(Mono.just(new ReactiveRedisConnection.KeyCommand(key))).flatMap(ReactiveRedisConnection.CommandResponse::getOutput);
	}
	 ...
	 * @see <a href="https://redis.io/commands/hgetall">Redis Documentation: HGETALL</a>
	 */
	Flux<ReactiveRedisConnection.CommandResponse<ReactiveRedisConnection.KeyCommand, Flux<Map.Entry<ByteBuffer, ByteBuffer>>>> hGetAll(Publisher<ReactiveRedisConnection.KeyCommand> commands);
	...
	default Flux<Map.Entry<ByteBuffer, ByteBuffer>> hScan(ByteBuffer key, ScanOptions options) {
		...
		return hScan(Mono.just(ReactiveRedisConnection.KeyScanCommand.key(key).withOptions(options))).map(ReactiveRedisConnection.CommandResponse::getOutput)
				.flatMap(it -> it);
	}
	 ...
	 *
	 * @param commands must not be {@literal null}.
	 * @return the {@link Flux} emitting {@link ReactiveRedisConnection.CommandResponse} one by one.
	 * @see <a href="https://redis.io/commands/hscan">Redis Documentation: HSCAN</a>
	 * @since 2.1
	 */
	Flux<ReactiveRedisConnection.CommandResponse<ReactiveRedisConnection.KeyCommand, Flux<Map.Entry<ByteBuffer, ByteBuffer>>>> hScan(Publisher<ReactiveRedisConnection.KeyScanCommand> commands);
	 ...
	 * @since 2.1
	 */
	class HStrLenCommand extends ReactiveRedisConnection.KeyCommand {
	...
	default Mono<Long> hStrLen(ByteBuffer key, ByteBuffer field) {
		...
		Assert.notNull(key, "Key must not be null!");
		Assert.notNull(field, "Field must not be null!");
		return hStrLen(Mono.just(HStrLenCommand.lengthOf(field).from(key))).next().map(ReactiveRedisConnection.NumericResponse::getOutput);
	}
	 ...
	 * @since 2.1
	 */
	Flux<ReactiveRedisConnection.NumericResponse<HStrLenCommand, Long>> hStrLen(Publisher<HStrLenCommand> commands);
}
