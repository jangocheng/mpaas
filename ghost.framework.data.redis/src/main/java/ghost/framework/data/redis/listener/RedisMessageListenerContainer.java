...
import ghost.framework.context.SmartLifecycle;
import ghost.framework.context.message.Topic;
import ghost.framework.context.scheduling.SchedulingAwareRunnable;
import ghost.framework.data.redis.connection.*;
import ghost.framework.data.redis.connection.util.ByteArrayWrapper;
 ...
 * @author Mark Paluch
 */
public class RedisMessageListenerContainer implements AutoCloseable, SmartLifecycle {
	 ...
	 * <p>
	 * The default implementation builds a {@link ghost.framework.core.task.SimpleAsyncTaskExecutor} with the
	 * specified bean name (or the class name, if no bean name specified) as thread name prefix.
	 *
	 * @see ghost.framework.core.task.SimpleAsyncTaskExecutor#SimpleAsyncTaskExecutor(String)
	 */
	...
	public void close() throws Exception {
		...
		if (manageExecutor) {
			if (taskExecutor instanceof AutoCloseable) {
				((AutoCloseable) taskExecutor).close();
...
//								connection.getSubscription().pSubscribe(unwrap(patternMapping.keySet()));
		...
		public void run() {
			...
			try {
				...
				boolean asyncConnection = false;//ConnectionUtils.isAsync(connectionFactory);
		 ...
		 *
		 * @author Thomas Darimont
		 * @see ghost.framework.data.redis.listener.RedisMessageListenerContainer.SubscriptionTask.SubscriptionPresentTestCondition
		 */
		private class PatternSubscriptionPresentCondition extends SubscriptionPresentCondition {

			@Override
			public boolean passes() {
				return false;//super.passes() && !CollectionUtils.isEmpty(connection.getSubscription().getPatterns());
			}
		}
		...
		void cancel() {
			...
			Subscription sub = null;//connection.getSubscription();
...
//						}
	 ...
	 * {@link ghost.framework.data.redis.listener.RedisMessageListenerContainer.SpinBarrier}
	 *
	 * @author Jennifer Hickey
	 * @author Thomas Darimont Note: Placed here to avoid API exposure.
	 */
