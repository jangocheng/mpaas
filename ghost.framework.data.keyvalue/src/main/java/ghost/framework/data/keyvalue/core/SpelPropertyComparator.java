...
public class SpelPropertyComparator<T> implements Comparator<T> {
	...
	protected String buildExpressionForPath() {
		...
		String rawExpression = String.format(
				"new ghost.framework.context.utils.comparator.NullSafeComparator(new ghost.framework.context.utils.comparator.ComparableComparator(), %s).compare(#arg1?.%s,#arg2?.%s)",
