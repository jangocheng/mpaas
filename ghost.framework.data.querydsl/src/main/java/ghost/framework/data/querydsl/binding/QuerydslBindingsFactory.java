...
import ghost.framework.data.commons.repository.support.Repositories;
...
//	@Override
	public void setApplicationContext(IApplication applicationContext) throws BeanException {
		...
		this.beanFactory = null;//Optional.of(applicationContext.getAutowireCapableBeanFactory());
		this.repositories = Optional.of(new Repositories(null));
	}
	...
	private QuerydslBinderCustomizer<EntityPath<?>> createQuerydslBinderCustomizer(
		...
		return beanFactory.map(it -> {
			...
			try {
				return null;//it.getBean(type);
			} catch (NoSuchBeanDefinitionException e) {
				return it.createBean(type);
			}
		}).orElseGet(() -> null/*BeanUtils.instantiateClass(type)*/);
	}
