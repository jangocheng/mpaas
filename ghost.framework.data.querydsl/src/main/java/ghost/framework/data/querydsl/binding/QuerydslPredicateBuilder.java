...
import ghost.framework.context.converter.Property;
...
public class QuerydslPredicateBuilder {
	...
	/**
	 * Creates a new {@link QuerydslPredicateBuilder} for the given {@link ConverterContainer} and
	 * {@link EntityPathResolver}.
	 *
	 * @param converterContainer must not be {@literal null}.
	 * @param resolver can be {@literal null}.
	 */
	public QuerydslPredicateBuilder(ConverterContainer converterContainer, EntityPathResolver resolver) {
	...
	private Collection<Object> convertToPropertyPathSpecificType(List<String> source, PathInformation path) {
		...
		for (String value : source) {
			...
			target.add(converterContainer.canConvert(String.class, targetType)
					? converterContainer.convert(value, TypeDescriptor.forObject(value), getTargetTypeDescriptor(path))
					: value);
		}
	...
	private static TypeDescriptor getTargetTypeDescriptor(PathInformation path) {
		...
		TypeDescriptor result = descriptor == null //
				? TypeDescriptor
						.nested(ghost.framework.data.commons.util.ReflectionUtils.findRequiredField(owningType, leafProperty), 0)
				: TypeDescriptor
