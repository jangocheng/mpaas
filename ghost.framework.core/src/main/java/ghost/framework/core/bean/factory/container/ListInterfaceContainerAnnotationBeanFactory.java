...
import ghost.framework.beans.annotation.container.BeanListInterfaceContainer;
...
import ghost.framework.context.bean.factory.container.IListInterfaceContainerAnnotationBeanFactory;
 ...
 * @Description:操作绑定 {@link java.util.List} 列表类型容器事件工厂类
...
public class ListInterfaceContainerAnnotationBeanFactory
        <
        ...
        implements IListInterfaceContainerAnnotationBeanFactory<O, T, E, V> {

    /**
     * 事件工厂注释
     */
    private final Class<? extends Annotation> annotation = BeanListInterfaceContainer.class;
    ...
    public String toString() {
        return "ListInterfaceContainerAnnotationBeanFactory{" +
    ...
    public void loader(E event) {
        //获取容器注释
        BeanListInterfaceContainer container = this.getAnnotation(event);
        BeanListContainer listContainer = container.container();
        ...
        //处理可以忽略找不到容器接口
        List list = event.getExecuteOwner().getNullableBean(listContainer.value());
        if (list != null) {
    ...
    public void unloader(E event) {
            //获取容器注释
        BeanListInterfaceContainer container = (BeanListInterfaceContainer) event.getTarget().getAnnotation(this.annotation);
        BeanListContainer listContainer = container.container();
        this.isInterface(container);
        //判断是否无效处理引发错误
        if (listContainer.error()) {
     ...
     * @param container
     */
    private void isInterface(BeanListInterfaceContainer container) {
