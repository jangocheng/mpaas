...
import ghost.framework.data.transaction.reactive.ReactiveTransactionCallback;
import ghost.framework.data.transaction.reactive.ReactiveTransactionManager;
import ghost.framework.data.transaction.reactive.ReactiveTransactionalOperator;
import ghost.framework.transaction.TransactionDefinition;
import ghost.framework.transaction.TransactionException;
import ghost.framework.transaction.TransactionSystemException;
 ...
 */
public final class ReactiveTransactionalOperatorImpl implements ReactiveTransactionalOperator {

	private static final Log logger = LogFactory.getLog(ReactiveTransactionalOperatorImpl.class);
	 ...
	 */
	public ReactiveTransactionalOperatorImpl(ReactiveTransactionManager transactionManager, TransactionDefinition transactionDefinition) {
	...
	public <T> Flux<T> execute(ReactiveTransactionCallback<T> action) throws TransactionException {
		return ReactiveTransactionContextManager.currentContext().flatMapMany(context -> {
			Mono<ReactiveTransaction> status = this.transactionManager.getReactiveTransaction(this.transactionDefinition);
			// This is an around advice: Invoke the next interceptor in the chain.
			// This will normally result in a target object being invoked.
			// Need re-wrapping of ReactiveTransaction until we get hold of the exception
	...
	public boolean equals(@Nullable Object other) {
		return (this == other || (super.equals(other) && (!(other instanceof ReactiveTransactionalOperatorImpl) ||
				getTransactionManager() == ((ReactiveTransactionalOperatorImpl) other).getTransactionManager())));
	}
