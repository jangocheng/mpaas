...
public abstract class AbstractReactiveTransactionManager implements ReactiveTransactionManager, Serializable {
	...
	public final Mono<ReactiveTransaction> getReactiveTransaction(@Nullable TransactionDefinition definition)
		...
		return ReactiveTransactionSynchronizationManager.forCurrentTransaction()
			...
			if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
				...
				return ReactiveTransactionContextManager.currentContext()
	 ...
	 */
	private Mono<ReactiveTransaction> handleExistingTransaction(ReactiveTransactionSynchronizationManager synchronizationManager,
	...
	public final Mono<Void> commit(ReactiveTransaction transaction) throws TransactionException {
		...
		return ReactiveTransactionSynchronizationManager.forCurrentTransaction().flatMap(synchronizationManager -> {
		...
		Mono<Object> commit = prepareForCommit(synchronizationManager, status)
				...
				.then(Mono.defer(() -> {
					...
					if (ErrorPredicates.UNEXPECTED_ROLLBACK.test(ex)) {
						result = triggerAfterCompletion(synchronizationManager, status, ReactiveTransactionSynchronization.STATUS_ROLLED_BACK).then(propagateException);
					}
					else if (ErrorPredicates.TRANSACTION_EXCEPTION.test(ex)) {
						result = triggerAfterCompletion(synchronizationManager, status, ReactiveTransactionSynchronization.STATUS_UNKNOWN).then(propagateException);
					}
	 ...
	 * given ReactiveTransactionSynchronization objects.
	 ...
	 * @throws ghost.framework.transaction.CannotCreateTransactionException if transaction support is not available
	 ...
	 * @throws ghost.framework.transaction.NestedTransactionNotSupportedException
	 ...
	 * @throws ghost.framework.transaction.TransactionSuspensionNotSupportedException if suspending is not supported by the transaction manager implementation
	 * @throws TransactionException in case of system errors
	 * @see #doResume
	 */
	 ...
	 * @throws ghost.framework.transaction.TransactionSuspensionNotSupportedException if resuming is not supported by the transaction manager implementation
	 * @throws TransactionException in case of system errors
	 * @see #doSuspend
	 */
