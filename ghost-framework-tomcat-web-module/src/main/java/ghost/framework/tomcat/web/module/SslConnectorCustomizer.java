...
class SslConnectorCustomizer implements TomcatConnectorCustomizer {
	...
	public SslConnectorCustomizer(ClassLoader classLoader, Ssl ssl, SslStoreProvider sslStoreProvider) {
		Assert.notNull(classLoader, "ClassLoader configuration should not be null");
	...
	private void configureSslKeyStore(AbstractHttp11JsseProtocol<?> protocol, Ssl ssl) {
		try {
			protocol.setKeystoreFile(ResourceUtils.getURL(this.classLoader, ssl.getKeyStore()).toString());
		}
		catch (Exception ex) {
	...
	private void configureSslTrustStore(AbstractHttp11JsseProtocol<?> protocol, Ssl ssl) {
		if (ssl.getTrustStore() != null) {
			try {
				protocol.setTruststoreFile(ResourceUtils.getURL(this.classLoader, ssl.getTrustStore()).toString());
			}
