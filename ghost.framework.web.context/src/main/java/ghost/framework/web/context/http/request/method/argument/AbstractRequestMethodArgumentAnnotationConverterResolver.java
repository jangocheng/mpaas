 ...
 * @Date: 2020/3/5:13:57
 */
public abstract class AbstractRequestMethodArgumentAnnotationConverterResolver
        extends AbstractRequestMethodArgumentAnnotationResolver
        implements IRequestMethodArgumentAnnotationConverterResolver {
    /**
     * 注入转化器工厂容器
     */
    @Autowired
    ...
    public Object resolveArgumentConverter(@NotNull HttpServletRequest request, @NotNull HttpServletResponse response,
        ...
        TypeConverter converter = container.getTargetTypeConverter(parameter.getType());
        if(converter instanceof EncodingTypeConverter) {
            return ((EncodingTypeConverter) converter).convert(value, request.getCharacterEncoding());
        }
