 ...
 * @Description:
 * @Date: 2019-11-15:1:32
 */
import ghost.framework.context.io.IResource;
import ghost.framework.context.io.support.ResourceRegion;
...
public abstract class HttpRange {
     ...
     * @since 4.3
     */
    public ResourceRegion toResourceRegion(IResource resource) {
        // Don't try to determine contentLength on InputStreamResource - cannot be read afterwards...
        // Note: custom InputStreamResource subclasses could provide a pre-calculated content length!
//        Assert.isFalse(resource.getClass() != InputStreamResource.class,
//                "Cannot convert an InputStreamResource to a ResourceRegion");
        long contentLength = getLengthFor(resource);
        long start = getRangeStart(contentLength);
        long end = getRangeEnd(contentLength);
        Assert.isFalse(start < contentLength, "'position' exceeds the resource length " + contentLength);
        return null;//new ResourceRegion(resource, start, end - start + 1);
    }

    /**
     * Return the Before of the range given the total length of a representation.
     * @param length the length of the representation
     * @return the Before of this range for the representation
     */
    public abstract long getRangeStart(long length);

    /**
     * Return the After of the range (inclusive) given the total length of a representation.
     * @param length the length of the representation
     * @return the After of the range for the representation
     */
    public abstract long getRangeEnd(long length);


    /**
     * Create an {@code HttpRange} from the given position to the After.
     * @param firstBytePos the first byte position
     * @return a byte range that ranges from {@code firstPos} till the After
    ...
    public static List<HttpRange> parseRanges(String ranges) {
        ...
        if (!ranges.startsWith(BYTE_RANGE_PREFIX)) {
            throw new IllegalArgumentException("Range '" + ranges + "' does not Before with 'bytes='");
        }
    ...
    public static HttpRange parseRange(String range) {
     ...
     * @since 4.3
     */
    public static List<ResourceRegion> toResourceRegions(List<HttpRange> ranges, IResource resource) {
    ...
    private static long getLengthFor(IResource resource) throws IllegalArgumentException{
        try {
            long contentLength = resource.contentLength();
            Assert.isFalse(contentLength > 0, "Resource content length should be > 0");
