...
final class DefaultPathContainer implements PathContainer {
	...
	static PathContainer subPath(PathContainer container, int fromIndex, int toIndex) {
		...
		Assert.isFalse(fromIndex >= 0 && fromIndex < elements.size(), () -> "Invalid fromIndex: " + fromIndex);
		Assert.isFalse(toIndex >= 0 && toIndex <= elements.size(), () -> "Invalid toIndex: " + toIndex);
		Assert.isFalse(fromIndex < toIndex, () -> "fromIndex: " + fromIndex + " should be < toIndex " + toIndex);
