...
import ghost.framework.beans.factory.BeanNameAware;
...
public abstract class AbstractView implements View, BeanNameAware, ApplicationContextAware {
	...
	/**
	 * Prepare the entity to render.
	 * @param model a map with attribute names as keys and corresponding entity
	 * objects as values (the map can also be {@code null} in case of an empty entity)
	...
	/**
	 * Prepare the entity to use for rendering.
	 * <p>The default implementation creates a combined output Map that includes
	 * entity as well as static attributes with the former taking precedence.
	 */
	 ...
	 * completes, the asynchronous attributes in the entity will have been
	 * replaced with their corresponding resolved values.
	 * @return result a {@code Mono} that completes when the entity is ready
	 * @since 5.1.8
	 */
	 ...
	 * attributes to {@code Mono<T>} or {@code Mono<List<T>>} and then wait to
	 * resolve them into actual values. When the returned {@code Mono<Void>}
	 * completes, the asynchronous attributes in the entity would have been
	 * replaced with their corresponding resolved values.
	 * @return result {@code Mono} that completes when the entity is ready
	 ...
	 * instance for the given exchange and entity.
