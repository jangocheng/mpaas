...
package ghost.framework.webflux.web.module.result.view.freemarker;
...
import ghost.framework.context.ApplicationContextException;
import ghost.framework.context.i18n.LocaleContextHolder;
import ghost.framework.core.io.buffer.DataBuffer;
import ghost.framework.core.io.buffer.DataBufferUtils;
import ghost.framework.core.io.buffer.PooledDataBuffer;
import ghost.framework.http.MediaType;
import ghost.framework.beans.annotation.constraints.Nullable;
import ghost.framework.util.Assert;
import ghost.framework.util.MimeType;
import ghost.framework.web.reactive.result.view.AbstractUrlBasedView;
import ghost.framework.web.reactive.result.view.RequestContext;
...
public class FreeMarkerView extends AbstractUrlBasedView {
	...
	/**
	 * Attribute name of the {@link RequestContext} instance in the template entity,
	 * available to Spring's macros &mdash; for example, for creating
	 * {@link ghost.framework.web.reactive.result.view.BindStatus BindStatus}
	 * objects.
	 * @since 5.2
	 ...
	 * @throws BeanException if no {@code FreeMarkerConfig} instance could be found
	 * @see #setConfiguration
	 */
	protected FreeMarkerConfig autodetectConfiguration() throws BeanException {
		try {
	...
	/**
	 * Prepare the entity to use for rendering by potentially exposing a
	 ...
	 * @since 5.2
	 * @see #setExposeSpringMacroHelpers(boolean)
	 * @see ghost.framework.web.reactive.result.view.AbstractView#getModelAttributes(Map, ServerWebExchange)
	 */
	@Override
		...
		if (this.exposeSpringMacroHelpers) {
			if (model != null && model.containsKey(SPRING_MACRO_REQUEST_CONTEXT_ATTRIBUTE)) {
				throw new IllegalStateException(
						...
						"' because of an existing entity object of the same name");
			}
			// Make a defensive copy of the entity.
	...
	/**
	 * Build a FreeMarker template entity for the given entity map.
	 * <p>The default implementation builds a {@link SimpleHash}.
	 * @param model the entity to use for rendering
	 * @param exchange current exchange
	 * @return the FreeMarker template entity, as a {@link SimpleHash} or subclass thereof
	 */
