    ...
    public void close() {
        if (closed.compareAndSet(false, true)) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.error(UpgradeUtil.getLocalContainer().getString("endpoint.debug.handlerRelease"), e);
                }
            } finally {
    ...
    protected abstract class OperationState<A> implements Runnable {
        ...
        protected boolean process() {
            try {
                ...
                log.warn(UpgradeUtil.getLocalContainer().getString("endpoint.executor.fail", SocketWrapperBase.this) , ree);
            } catch (Throwable t) {
                ExceptionUtils.handleThrowable(t);
                // This means we got an OOM or similar creating a thread, or that
                // the pool and its queue are full
                log.error(UpgradeUtil.getLocalContainer().getString("endpoint.process.fail"), t);
            }
            return false;
        }
