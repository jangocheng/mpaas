        ...
        while (true) {
            ...
            default:
                throw new IllegalStateException(
                        UpgradeUtil.getLocalContainer().getString("wsFrameServer.illegalReadState", getReadState()));
            }
        }
    }
    ...
    private class WsFrameClientCompletionHandler implements CompletionHandler<Integer, Void> {
        ...
        private void doResumeProcessing(boolean checkOpenOnError) {
            while (true) {
                ...
                default:
                    throw new IllegalStateException(
                            UpgradeUtil.getLocalContainer().getString("wsFrame.illegalReadState", getReadState()));
                }
            }
        }
    }
            ...
            if (checkOpenOnError) {
                ...
                if (isOpen()) {
                    if (log.isDebugEnabled()) {
                        log.debug(UpgradeUtil.getLocalContainer().getString("wsFrameClient.ioe"), e);
                    }
                    close(e);
                }
            } else {
