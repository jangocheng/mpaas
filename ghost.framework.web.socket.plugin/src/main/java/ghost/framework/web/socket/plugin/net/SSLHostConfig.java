...
import ghost.framework.web.socket.plugin.net.openssl.OpenSSLConf;
import ghost.framework.web.socket.plugin.net.openssl.ciphers.Cipher;
...
public class SSLHostConfig implements Serializable {
    ...
    boolean setProperty(String name, Type configType) {
        if (this.configType == null) {
            ...
            if (configType != this.configType) {
                log.warn(UpgradeUtil.getLocalContainer().getString("sslHostConfig.mismatch",
    ...
    public void addCertificate(SSLHostConfigCertificate certificate) {
        ...
        if (certificates.size() == 1 &&
            ...
            // Invalid config
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("sslHostConfig.certificate.notype"));
        }
    ...
    public void setOpenSslConf(OpenSSLConf conf) {
        if (conf == null) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("sslHostConfig.opensslconf.null"));
        } else if (openSslConf != null) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("sslHostConfig.opensslconf.alreadySet"));
        }
            ...
            if (trimmed.length() > 1) {
                if (trimmed.charAt(0) == '+') {
                    ...
                    if (!protocols.isEmpty()) {
                        log.warn(UpgradeUtil.getLocalContainer().getString("sslHostConfig.prefix_missing",
                                 trimmed, getHostName()));
                    }
                    if (trimmed.equalsIgnoreCase(Constants.SSL_PROTO_ALL)) {
                        protocols.addAll(SSL_PROTO_ALL_SET);
            ...
            if (truststoreFile != null){
                try {
                    ...
                    if (cause instanceof UnrecoverableKeyException) {
                        ...
                        log.warn(UpgradeUtil.getLocalContainer().getString("jsse.invalid_truststore_password"),
                                cause);
                        // Re-try
                        result = SSLUtilBase.getStore(getTruststoreType(), getTruststoreProvider(),
    ...
    public static String adjustRelativePath(String path) throws FileNotFoundException {
        ...
        if (!f.exists()) {
            throw new FileNotFoundException(UpgradeUtil.getLocalContainer().getString("sslHostConfig.fileNotFound", newPath));
        }
        return newPath;
    }
        ...
        public static CertificateVerification fromString(String value) {
            if ("true".equalsIgnoreCase(value) ||
                ...
                throw new IllegalArgumentException(
                        UpgradeUtil.getLocalContainer().getString("sslHostConfig.certificateVerificationInvalid", value));
            }
        }
    }
}
