...
package ghost.framework.web.socket.plugin.net.openssl.ciphers;
...
public class OpenSSLCipherConfigurationParser {
        ...
        for (String element : elements) {
            if (element.startsWith(DELETE)) {
                ...
                if (aliases.containsKey(alias)) {
                    ...
                    log.warn(UpgradeUtil.getLocalContainer().getString("jsse.openssl.unknownElement", alias));
                }
    ...
    public static List<String> convertForJSSE(Collection<Cipher> ciphers) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(UpgradeUtil.getLocalContainer().getString("jsse.openssl.effectiveCiphers", displayResult(ciphers, true, ",")));
        }
        return result;
    }
