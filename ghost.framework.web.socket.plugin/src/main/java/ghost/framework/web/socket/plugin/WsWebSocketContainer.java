...
import ghost.framework.web.socket.plugin.pojo.PojoEndpointClient;
...
public class WsWebSocketContainer implements WebSocketContainer, BackgroundProcess {
    ...
    /**
     * 客户端连接服务器
     * @param pojo {@link ClientEndpoint} 注释的对象
     * @param path 连接服务器地址
     * @return 返回连接完成的会话对象
     * @throws DeploymentException 部署错误
     */
    @Override
    public Session connectToServer(Object pojo, URI path) throws DeploymentException {
        ...
        if (annotation == null) {
            throw new DeploymentException(
                    UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.missingAnnotation",
    ...
    private Session connectToServerRecursive(Endpoint endpoint,
        ...
        if (host == null) {
            throw new DeploymentException(
                    UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.pathNoHost"));
        }
        ...
        if (proxyConnect != null) {
            try {
                ...
                throw new DeploymentException(
                        UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.httpRequestFailed"), e);
            }
        }
        ...
        try {
            ...
            if (httpResponse.status != 101) {
                if(isRedirectStatus(httpResponse.status)){
                    ...
                    if (auth == null) {
                        throw new DeploymentException(
                                UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.unsupportedAuthScheme",
                    ...
                    throw new DeploymentException(UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.invalidStatus",
                            Integer.toString(httpResponse.status)));
                }
            }
            ...
            if (protocolHeaders == null || protocolHeaders.size() == 0) {
                ...
                throw new DeploymentException(
                        UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.invalidSubProtocol"));
            }
    ...
    /**
     * 注册会话
     * @param key 会话Endpoint键
     * @param wsSession 会话对象
     */
...
//                BackgroundProcessManager.getInstance().unregister(this);
    ...
    private static Map<String, List<String>> createRequestHeaders(String host, int port, boolean secure, ClientEndpointConfig clientEndpointConfiguration) {
        ...
        headers.put(HandshakeRequest.SEC_WEBSOCKET_VERSION, wsVersionValues);

        // WebSocket key
        List<String> wsKeyValues = new ArrayList<>(1);
        wsKeyValues.add(generateWsKeyValue());
        headers.put(HandshakeRequest.SEC_WEBSOCKET_KEY, wsKeyValues);

        // WebSocket sub-protocols
        if (subProtocols != null && subProtocols.size() > 0) {
            headers.put(HandshakeRequest.SEC_WEBSOCKET_PROTOCOL, subProtocols);
        }

        // WebSocket extensions
        if (extensions != null && extensions.size() > 0) {
            headers.put(HandshakeRequest.SEC_WEBSOCKET_EXTENSIONS,
    ...
    private void parseHeaders(String line, Map<String,List<String>> headers) {
        ...
        if (index == -1) {
            log.warn(UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.invalidHeader", line));
            return;
        }
    ...
    public void destroy() {
        CloseReason cr = new CloseReason(
                CloseReason.CloseCodes.GOING_AWAY, UpgradeUtil.getLocalContainer().getString("wsWebSocketContainer.shutdown"));

        for (WsSession session : sessions.keySet()) {
            try {
                session.close(cr);
            } catch (IOException ioe) {
                log.debug(UpgradeUtil.getLocalContainer().getString(
