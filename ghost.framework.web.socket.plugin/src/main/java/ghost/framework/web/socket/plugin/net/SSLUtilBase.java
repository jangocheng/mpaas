...
import ghost.framework.web.socket.plugin.net.jsse.PEMFile;
import ghost.framework.web.socket.plugin.security.KeyStoreUtil;
import ghost.framework.web.socket.plugin.server.UpgradeUtil;
import ghost.framework.web.socket.plugin.util.file.ConfigFileLoader;
...
public abstract class SSLUtilBase implements SSLUtil {
    ...
    protected SSLUtilBase(SSLHostConfigCertificate certificate, boolean warnTls13) {
        ...
        if (enabledProtocols.contains("SSLv3")) {
            log.warn(UpgradeUtil.getLocalContainer().getString("jsse.ssl3"));
        }
        this.enabledProtocols = enabledProtocols.toArray(new String[0]);

        if (enabledProtocols.contains(Constants.SSL_PROTO_TLSv1_3) &&
                sslHostConfig.getCertificateVerification() == SSLHostConfig.CertificateVerification.OPTIONAL &&
                !isTls13RenegAuthAvailable() && warnTls13) {
            log.warn(UpgradeUtil.getLocalContainer().getString("jsse.tls13.auth"));
        }
        ...
        if (implemented.size() == 0) {
            ...
            if (enabled.isEmpty()) {
                ...
                throw new IllegalArgumentException(
                        UpgradeUtil.getLocalContainer().getString("sslUtilBase.noneSupported", name, configured));
            }
            if (log.isDebugEnabled()) {
                log.debug(UpgradeUtil.getLocalContainer().getString("sslUtilBase.active", name, enabled));
            }
            if (log.isDebugEnabled() || warnOnSkip) {
                if (enabled.size() != configured.size()) {
                    ...
                    String msg = UpgradeUtil.getLocalContainer().getString("sslUtilBase.skipped", name, skipped);
                    if (warnOnSkip) {
                        log.warn(msg);
                    } else {
                        log.debug(msg);
    ...
    static KeyStore getStore(String type, String provider, String path,
        ...
        try {
            ...
            String msg = UpgradeUtil.getLocalContainer().getString("jsse.keystore_load_failed", type, path,
                    ex.getMessage());
            log.error(msg, ex);
    ...
    public KeyManager[] getKeyManagers() throws Exception {
        ...
        if (ks == null) {
            if (certificate.getCertificateFile() == null) {
                throw new IOException(UpgradeUtil.getLocalContainer().getString("jsse.noCertFile"));
            }
            ...
            if (keyAlias != null && !ks.isKeyEntry(keyAlias)) {
                throw new IOException(UpgradeUtil.getLocalContainer().getString("jsse.alias_no_key_entry", keyAlias));
            } else if (keyAlias == null) {
                Enumeration<String> aliases = ks.aliases();
                if (!aliases.hasMoreElements()) {
                    throw new IOException(UpgradeUtil.getLocalContainer().getString("jsse.noKeys"));
                }
                ...
                if (keyAlias == null) {
                    throw new IOException(UpgradeUtil.getLocalContainer().getString("jsse.alias_no_key_entry", (Object) null));
                }
            }
    ...
    public TrustManager[] getTrustManagers() throws Exception {
        ...
        if(className != null && className.length() > 0) {
             ...
             if(!(TrustManager.class.isAssignableFrom(clazz))){
                throw new InstantiationException(UpgradeUtil.getLocalContainer().getString(
        ...
        if (trustStore != null) {
            ...
            if ("PKIX".equalsIgnoreCase(algorithm)) {
                ...
                if (crlf != null && crlf.length() > 0) {
                    throw new CRLException(UpgradeUtil.getLocalContainer().getString("jsseUtil.noCrlSupport", algorithm));
                }
                // Only warn if the attribute has been explicitly configured
                if (sslHostConfig.isCertificateVerificationDepthConfigured()) {
                    log.warn(UpgradeUtil.getLocalContainer().getString("jsseUtil.noVerificationDepth", algorithm));
                }
            }
        }
                ...
                if (trustStore.isCertificateEntry(alias)) {
                    ...
                    if (cert instanceof X509Certificate) {
                        try {
                            ...
                            String msg = UpgradeUtil.getLocalContainer().getString("jsseUtil.trustedCertNotValid", alias,
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug(UpgradeUtil.getLocalContainer().getString("jsseUtil.trustedCertNotChecked", alias));
                        }
                    }
                }
            }
