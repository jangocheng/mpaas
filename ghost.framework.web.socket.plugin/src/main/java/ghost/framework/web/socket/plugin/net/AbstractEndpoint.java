...
import ghost.framework.web.socket.plugin.collections.SynchronizedStack;
import ghost.framework.web.socket.plugin.server.UpgradeUtil;
import ghost.framework.web.socket.plugin.util.ExceptionUtils;
import ghost.framework.web.socket.plugin.util.IntrospectionUtils;
import ghost.framework.web.socket.plugin.util.threads.LimitLatch;
import ghost.framework.web.socket.plugin.util.threads.ResizableExecutor;
import ghost.framework.web.socket.plugin.util.threads.TaskThreadFactory;
...
public abstract class AbstractEndpoint<S,U> {
    ...
    public void addSslHostConfig(SSLHostConfig sslHostConfig, boolean replace) throws IllegalArgumentException {
        ...
        if (key == null || key.length() == 0) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("endpoint.noSslHostName"));
        }
        ...
        if (replace) {
            ...
            if (duplicate != null) {
                ...
                throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("endpoint.duplicateSslHostName", key));
            }
            registerJmx(sslHostConfig);
        }
    }
    ...
    public SSLHostConfig removeSslHostConfig(String hostName) {
        ...
        if (hostName.equalsIgnoreCase(getDefaultSSLHostConfigName())) {
            throw new IllegalArgumentException(
                    UpgradeUtil.getLocalContainer().getString("endpoint.removeDefaultSslHostConfig", hostName));
        }
    ...
    public void reloadSslHostConfig(String hostName) {
        ...
        if (sslHostConfig == null) {
            throw new IllegalArgumentException(
                    UpgradeUtil.getLocalContainer().getString("endpoint.unknownSslHostName", hostName));
        }
    ...
    public ScheduledExecutorService getUtilityExecutor() {
        if (utilityExecutor == null) {
            getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.warn.noUtilityExecutor"));
    ...
    public void setPortOffset(int portOffset ) {
        if (portOffset < 0) {
            throw new IllegalArgumentException(
                    UpgradeUtil.getLocalContainer().getString("endpoint.portOffset.invalid", Integer.valueOf(portOffset)));
        }
    ...
    public void setAttribute(String name, Object value) {
        if (getLog().isTraceEnabled()) {
            getLog().trace(UpgradeUtil.getLocalContainer().getString("endpoint.setAttribute", name, value));
        }
    ...
    public Object getAttribute(String key) {
        ...
        if (getLog().isTraceEnabled()) {
            getLog().trace(UpgradeUtil.getLocalContainer().getString("endpoint.getAttribute", key, value));
        }
        return value;
    }
    ...
    public boolean setProperty(String name, String value) {
        ...
        try {
            ...
            getLog().error(UpgradeUtil.getLocalContainer().getString("endpoint.setAttributeError", name, value), x);
            return false;
        }
    }
    ...
    public void createExecutor() {
        ...
        ghost.framework.web.socket.plugin.util.threads.TaskQueue taskqueue = new ghost.framework.web.socket.plugin.util.threads.TaskQueue();
        TaskThreadFactory tf = new TaskThreadFactory(getName() + "-exec-", daemon, getThreadPriority());
        executor = new ThreadPoolExecutor(getMinSpareThreads(), getMaxThreads(), 60, TimeUnit.SECONDS,taskqueue, tf);
        taskqueue.setParent( (ghost.framework.web.socket.plugin.util.threads.ThreadPoolExecutor) executor);
    }
            ...
            if (executor instanceof ThreadPoolExecutor) {
                ...
                if (timeout > 0) {
                    ...
                    if (tpe.isTerminating()) {
                        getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.warn.executorShutdown", getName()));
                    }
                }
                ghost.framework.web.socket.plugin.util.threads.TaskQueue queue = (ghost.framework.web.socket.plugin.util.threads.TaskQueue) tpe.getQueue();
    ...
    private void unlockAccept() {
        ...
        try {
            ...
            getLog().debug(UpgradeUtil.getLocalContainer().getString("endpoint.debug.unlock.localFail", getName()), ioe);
        }
        if (localAddress == null) {
            getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.debug.unlock.localNone", getName()));
            return;
        }
        ...
        try {
            ...
            if (getLog().isDebugEnabled()) {
                getLog().debug(UpgradeUtil.getLocalContainer().getString(
    ...
    public boolean processSocket(SocketWrapperBase<S> socketWrapper,
        ...
        try {
            ...
            getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.executor.fail", socketWrapper) , ree);
            return false;
        } catch (Throwable t) {
            ExceptionUtils.handleThrowable(t);
            // This means we got an OOM or similar creating a thread, or that
            // the pool and its queue are full
            getLog().error(UpgradeUtil.getLocalContainer().getString("endpoint.process.fail"), t);
            return false;
        }
        return true;
    }
...
//                Registry.getRegistry(null, null).registerComponent(sslHostConfig, sslOname, null);
                ...
                getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.jmxRegistrationFailed", sslOname), e);
            }
        } catch (MalformedObjectNameException e) {
            getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.invalidJmxNameSslHost",
...
//                            sslHostConfigCert, sslCertOname, null);
                    ...
                    getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.jmxRegistrationFailed", sslCertOname), e);
                }
            } catch (MalformedObjectNameException e) {
                getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.invalidJmxNameSslHostCert",
...
//        registry.unregisterComponent(socketProperties.getObjectName());
    ...
    protected long countDownConnection() {
        ...
        if (latch!=null) {
            ...
            if (result<0) {
                getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.warn.incorrectConnectionCount"));
            }
            return result;
        } else return -1;
    }
    ...
    public final void closeServerSocketGraceful() {
        if (bindState == BindState.BOUND_ON_START) {
            ...
            try {
                ...
                getLog().warn(UpgradeUtil.getLocalContainer().getString("endpoint.serverSocket.closeFailed", getName()), ioe);
            }
        }
    }
