 ...
 * ws客户端
 */
final class WsRemoteEndpointImplClient extends WsRemoteEndpointImplBase {
    ...
    protected void doWrite(SendHandler handler, long blockingWriteTimeoutExpiry,
        ...
        for (ByteBuffer byteBuffer : data) {
            if (blockingWriteTimeoutExpiry == -1) {
                ...
                if (timeout < 0) {
                    SendResult sr = new SendResult(new IOException(UpgradeUtil.getLocalContainer().getString("wsRemoteEndpoint.writeTimeout")));
                    handler.onResult(sr);
                }
            }
