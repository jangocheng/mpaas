...
import ghost.framework.web.socket.plugin.net.openssl.ciphers.OpenSSLCipherConfigurationParser;
import ghost.framework.web.socket.plugin.server.UpgradeUtil;
import ghost.framework.web.socket.plugin.util.buf.ByteBufferUtils;
import ghost.framework.web.socket.plugin.net.Constants;
import ghost.framework.web.socket.plugin.jni.Buffer;
import ghost.framework.web.socket.plugin.jni.Pool;
import ghost.framework.web.socket.plugin.jni.SSL;
import ghost.framework.web.socket.plugin.jni.SSLContext;
...
public final class OpenSSLEngine extends SSLEngine implements SSLUtil.ProtocolInfo {
    ...
    static {
        ...
        try {
            ...
            logger.warn(UpgradeUtil.getLocalContainer().getString("engine.ciphersFailure"), e);
        } finally {
    ...
    OpenSSLEngine(long sslCtx, String fallbackApplicationProtocol,
        ...
        if (sslCtx == 0) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.noSSLContext"));
        }
    ...
    private static int writePlaintextData(final long ssl, final ByteBuffer src) {
        ...
        throw new IllegalStateException(
                UpgradeUtil.getLocalContainer().getString("engine.writeToSSLFailed", Integer.toString(sslWrote)));
    }
    ...
    public synchronized SSLEngineResult wrap(final ByteBuffer[] srcs, final int offset, final int length, final ByteBuffer dst) throws SSLException {
        ...
        if (srcs == null || dst == null) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.nullBuffer"));
        }
        if (offset >= srcs.length || offset + length > srcs.length) {
            throw new IndexOutOfBoundsException(UpgradeUtil.getLocalContainer().getString("engine.invalidBufferArray",
        ...
        for (int i = offset; i < endOffset; ++i) {
            ...
            if (src == null) {
                throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.nullBufferInArray"));
            }
    ...
    public synchronized SSLEngineResult unwrap(final ByteBuffer src, final ByteBuffer[] dsts, final int offset, final int length) throws SSLException {
        ...
        if (len > MAX_ENCRYPTED_PACKET_LENGTH) {
            ...
            engineClosed = true;
            shutdown();
            throw new SSLException(UpgradeUtil.getLocalContainer().getString("engine.oversizedPacket"));
        }
    ...
    public synchronized void closeInbound() throws SSLException {
        ...
        if (accepted != Accepted.NOT && !receivedShutdown) {
            throw new SSLException(UpgradeUtil.getLocalContainer().getString("engine.inboundClose"));
        }
    }
    ...
    public synchronized void setEnabledCipherSuites(String[] cipherSuites) {
        ...
        if (cipherSuites == null) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.nullCipherSuite"));
        }
        if (destroyed) {
            return;
        }
        ...
        for (String cipherSuite : cipherSuites) {
            ...
            if (!AVAILABLE_CIPHER_SUITES.contains(cipherSuite)) {
                logger.debug(UpgradeUtil.getLocalContainer().getString("engine.unsupportedCipher", cipherSuite, converted));
            }
            if (converted != null) {
        ...
        if (buf.length() == 0) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.emptyCipherSuite"));
        }
        ...
        try {
            ...
            throw new IllegalStateException(UpgradeUtil.getLocalContainer().getString("engine.failedCipherSuite", cipherSuiteSpec), e);
        }
    }
    ...
    public synchronized void setEnabledProtocols(String[] protocols) {
        ...
        for (String p : protocols) {
            if (!IMPLEMENTED_PROTOCOLS_SET.contains(p)) {
                throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.unsupportedProtocol", p));
            }
    ...
    public synchronized void beginHandshake() throws SSLException {
        if (engineClosed || destroyed) {
            throw new SSLException(UpgradeUtil.getLocalContainer().getString("engine.engineClosed"));
        }
        switch (accepted) {
        case NOT:
            handshake();
    ...
    private void checkLastError() throws SSLException {
        ...
        if (error != SSL.SSL_ERROR_NONE) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(UpgradeUtil.getLocalContainer().getString("engine.openSSLError", Long.toString(error), err));
            }
    ...
    public void setEnableSessionCreation(boolean b) {
        if (!b) {
            String msg = UpgradeUtil.getLocalContainer().getString("engine.noRestrictSessionCreation");
    ...
    private class OpenSSLSession implements SSLSession {
        ...
        public void putValue(String name, Object value) {
            if (name == null) {
                throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.nullName"));
            }
            if (value == null) {
                throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("engine.nullValue"));
            }
            ...
            if (c == null) {
                ...
                synchronized (OpenSSLEngine.this) {
                    if (destroyed || SSL.isInInit(ssl) != 0) {
                        throw new SSLPeerUnverifiedException(UpgradeUtil.getLocalContainer().getString("engine.unverifiedPeer"));
                    }
