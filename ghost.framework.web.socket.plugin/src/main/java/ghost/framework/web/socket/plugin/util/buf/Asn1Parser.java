 ...
 */
package ghost.framework.web.socket.plugin.util.buf;
...
public class Asn1Parser {
    ...
    public void parseTag(int tag) {
        int value = next();
        if (value != tag) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("asn1Parser.tagMismatch",
    ...
    public void parseFullLength() {
        ...
        if (len + pos != source.length) {
            throw new IllegalArgumentException(UpgradeUtil.getLocalContainer().getString("asn1Parser.lengthInvalid",
