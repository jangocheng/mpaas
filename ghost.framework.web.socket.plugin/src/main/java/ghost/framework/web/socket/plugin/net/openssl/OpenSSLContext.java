...
import ghost.framework.web.socket.plugin.jni.*;
import ghost.framework.web.socket.plugin.net.Constants;
import ghost.framework.web.socket.plugin.net.SSLHostConfig;
...
public class OpenSSLContext implements ghost.framework.web.socket.plugin.net.SSLContext {
    ...
    static {
        try {
            ...
            throw new IllegalStateException(UpgradeUtil.getLocalContainer().getString("openssl.X509FactoryError"), e);
        }
    }
        ...
        try {
            ...
            if (openSslConf != null) {
                try {
                    if (log.isDebugEnabled())
                        log.debug(UpgradeUtil.getLocalContainer().getString("openssl.makeConf"));
                    ...
                    throw new SSLException(UpgradeUtil.getLocalContainer().getString("openssl.errMakeConf"), e);
                }
            } else {
                cctx = 0;
            }
            ...
            for (String protocol : sslHostConfig.getEnabledProtocols()) {
                if (Constants.SSL_PROTO_SSLv2Hello.equalsIgnoreCase(protocol)) {
                    ...
                    throw new Exception(UpgradeUtil.getLocalContainer().getString(
            ...
            try {
                ...
                throw new Exception(
                        UpgradeUtil.getLocalContainer().getString("endpoint.apr.failSslContextMake"), e);
            }
            ...
            throw new SSLException(UpgradeUtil.getLocalContainer().getString("openssl.errorSSLCtxInit"), e);
        } finally {
            if (!success) {
                destroy();
            }
    ...
    public synchronized void init(KeyManager[] kms, TrustManager[] tms, SecureRandom sr) {
        if (initialized) {
            log.warn(UpgradeUtil.getLocalContainer().getString("openssl.doubleInit"));
            return;
        }
        try {
            ...
            if (tms != null) {
                ...
                SSLContext.setCertVerifyCallback(ctx, new CertificateVerifier() {
                    ...
                    public boolean verify(long ssl, byte[][] chain, String auth) {
                        ...
                        try {
                            ...
                            log.debug(UpgradeUtil.getLocalContainer().getString("openssl.certificateVerificationFailed"), e);
                        }
                        return false;
                    }
                });
                ...
                for (X509Certificate caCert : x509TrustManager.getAcceptedIssuers()) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug(UpgradeUtil.getLocalContainer().getString("openssl.addedClientCaCert", caCert.toString()));
                }
            } else {
            ...
            if (openSslConf != null && cctx != 0) {
                ...
                if (log.isDebugEnabled())
                    log.debug(UpgradeUtil.getLocalContainer().getString("openssl.checkConf"));
                try {
                    if (!openSslConf.check(cctx)) {
                        log.error(UpgradeUtil.getLocalContainer().getString("openssl.errCheckConf"));
                        throw new Exception(UpgradeUtil.getLocalContainer().getString("openssl.errCheckConf"));
                    }
                } catch (Exception e) {
                    throw new Exception(UpgradeUtil.getLocalContainer().getString("openssl.errCheckConf"), e);
                }
                if (log.isDebugEnabled())
                    log.debug(UpgradeUtil.getLocalContainer().getString("openssl.applyConf"));
                try {
                    if (!openSslConf.apply(cctx, ctx)) {
                        log.error(UpgradeUtil.getLocalContainer().getString("openssl.errApplyConf"));
                        throw new SSLException(UpgradeUtil.getLocalContainer().getString("openssl.errApplyConf"));
                    }
                } catch (Exception e) {
                    throw new SSLException(UpgradeUtil.getLocalContainer().getString("openssl.errApplyConf"), e);
                }
            ...
            log.warn(UpgradeUtil.getLocalContainer().getString("openssl.errorSSLCtxInit"), e);
            destroy();
        }
    }
    ...
    private static X509TrustManager chooseTrustManager(TrustManager[] managers) {
        ...
        throw new IllegalStateException(UpgradeUtil.getLocalContainer().getString("openssl.trustManagerMissing"));
    }
    ...
    public SSLEngine createSSLEngine() {
        return new OpenSSLEngine(ctx, defaultProtocol, false, sessionContext,
                ...
                sslHostConfig.getCertificateVerification() == SSLHostConfig.CertificateVerification.OPTIONAL_NO_CA);
    }
