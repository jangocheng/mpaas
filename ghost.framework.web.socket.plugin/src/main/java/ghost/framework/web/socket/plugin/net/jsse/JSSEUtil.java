...
import ghost.framework.web.socket.plugin.util.compat.JreVendor;
            ...
            synchronized (this) {
                if (!initialized) {
                    ...
                    for (String protocol : implementedProtocolsArray) {
                        ...
                        if (!"SSLV2HELLO".equals(protocolUpper) && !"SSLV3".equals(protocolUpper)) {
                            if (protocolUpper.contains("SSL")) {
                                log.debug(UpgradeUtil.getLocalContainer().getString("jsse.excludeProtocol", protocol));
                                continue;
                            }
                        }
                    ...
                    if (implementedProtocols.size() == 0) {
                        log.warn(UpgradeUtil.getLocalContainer().getString("jsse.noDefaultProtocols"));
                    }
