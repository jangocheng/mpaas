...
//import ghost.framework.core.InfrastructureProxy;
...
//import ghost.framework.transaction.jta.JtaTransactionManager;
 ...
 * @see #LocalSessionFactoryBuilder(DataSource, IResourceLoader, MetadataSources)
...
public class LocalSessionFactoryBuilder extends Configuration {
	...
	public LocalSessionFactoryBuilder(@Nullable DataSource dataSource) {
		this(dataSource, new PathMatchingIResourcePatternResolver());
	}
	...
	public LocalSessionFactoryBuilder(@Nullable DataSource dataSource, ClassLoader classLoader) {
		this(dataSource, new PathMatchingIResourcePatternResolver(classLoader));
	}
	 ...
	 * @param dataSource the JDBC DataSource that the resulting Hibernate SessionFactory should be using
	 * (may be {@code null})
	 * @param IResourceLoader the IResourceLoader to load application classes from
	 */
	public LocalSessionFactoryBuilder(@Nullable DataSource dataSource, IResourceLoader IResourceLoader) {
		this(dataSource, IResourceLoader, new MetadataSources(
				new BootstrapServiceRegistryBuilder().applyClassLoader(IResourceLoader.getClassLoader()).build()));
	}
	...
	public LocalSessionFactoryBuilder(
			@Nullable DataSource dataSource, IResourceLoader IResourceLoader, MetadataSources metadataSources) {
		...
		getProperties().put(AvailableSettings.CLASSLOADERS, Collections.singleton(IResourceLoader.getClassLoader()));
	 ...
	 * (e.g. a {@link ghost.framework.core.task.SimpleAsyncTaskExecutor}).
