...
package ghost.framework.data.orm.jpa.persistenceunit;

import ghost.framework.beans.annotation.constraints.Nullable;
import ghost.framework.beans.annotation.invoke.Loader;
import ghost.framework.context.bean.CandidateComponentsIndex;
import ghost.framework.context.bean.CandidateComponentsIndexLoader;
...
import ghost.framework.context.instrument.classloading.InstrumentationLoadTimeWeaver;
import ghost.framework.context.instrument.classloading.LoadTimeWeaver;
import ghost.framework.context.instrument.classloading.ReflectiveLoadTimeWeaver;
...
import ghost.framework.data.jdbc.datasource.lookup.BeanFactoryDataSourceLookup;
import ghost.framework.data.jdbc.datasource.lookup.DataSourceLookup;
import ghost.framework.data.jdbc.datasource.lookup.JndiDataSourceLookup;
import ghost.framework.data.jdbc.datasource.lookup.MapDataSourceLookup;
import ghost.framework.data.orm.jpa.JpaVendorAdapter;
...
//import ghost.framework.context.index.CandidateComponentsIndex;
//import ghost.framework.context.index.CandidateComponentsIndexLoader;
...
//import ghost.framework.instrument.classloading.InstrumentationLoadTimeWeaver;
//import ghost.framework.instrument.classloading.LoadTimeWeaver;
//import ghost.framework.jdbc.datasource.lookup.DataSourceLookup;
//import ghost.framework.jdbc.datasource.lookup.JndiDataSourceLookup;
//import ghost.framework.jdbc.datasource.lookup.MapDataSourceLookup;
 ...
 * @see ghost.framework.orm.jpa.LocalContainerEntityManagerFactoryBean#setPersistenceUnitManager
 */
public class DefaultPersistenceUnitManager
		implements PersistenceUnitManager/*, ResourceLoaderAware, LoadTimeWeaverAware */{
...
//	public static final String DEFAULT_PERSISTENCE_XML_LOCATION = "classpath*:META-INF/" + PERSISTENCE_XML_FILENAME;
	public static final String DEFAULT_PERSISTENCE_XML_LOCATION = "classpath*:" + PERSISTENCE_XML_FILENAME;
	/**
	...
	private IResourcePatternResolver resourcePatternResolver = new PathMatchingIResourcePatternResolver();
	 ...
	 * @see BeanFactoryDataSourceLookup
	 * @see #setDataSources
...
//	@Override
	public void setResourceLoader(IResourceLoader IResourceLoader) {
		this.resourcePatternResolver = ResourcePatternUtils.getResourcePatternResolver(IResourceLoader);
		this.componentsIndex = CandidateComponentsIndexLoader.loadIndex(IResourceLoader.getClassLoader());
	}
	...
	private SpringPersistenceUnitInfo buildDefaultPersistenceUnitInfo() {
		...
		if (this.mappingResources != null) {
			...
			IResource ormXml = getOrmXmlForDefaultPersistenceUnit();
			if (ormXml != null) {
	...
	private void scanPackage(SpringPersistenceUnitInfo scannedUnit, String pkg) {
		...
		try {
			String pattern = IResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX +
...
//			throw new PersistenceException("Unable to resolve persistence unit root URL", ex);
	 ...
	 */
	@Nullable
	private IResource getOrmXmlForDefaultPersistenceUnit() {
		IResource ormXml = this.resourcePatternResolver.getResource(
				this.defaultPersistenceUnitRootLocation + DEFAULT_ORM_XML_RESOURCE);
		if (ormXml.exists()) {
			try {
				IResource persistenceXml = ormXml.createRelative(PERSISTENCE_XML_FILENAME);
