...
import java.util.*;
/**
 * {@link ghost.framework.web.module.servlet.server.ServletWebServerFactory} that can be used to create a {@link JettyWebServer}.
...
@Order(0)
@Component
public class JettyServletWebServerFactory extends AbstractServletWebServerFactory implements ConfigurableJettyWebServerFactory {
	/**
	 * 注入模块接口
	 */
	@Autowired
	...
	public WebServer getWebServer(
			@Module(name = JettyWebModuleConstant.ModuleWebName)
			@Autowired("ghost.framework.web.module.servlet.context.ServletContextInitializerContainer") Collection<ServletContextInitializer> initializers) {
	 ...
	 */
	protected final void configureWebAppContext(WebAppContext context, Collection<ServletContextInitializer> initializers) {
		Assert.notNull(context, "Context must not be null");
		context.getAliasChecks().clear();
		context.setTempDirectory(getTempDirectory());
		context.setClassLoader((ClassLoader) this.module.getClassLoader());
	...
	private static final class LoaderHidingResource extends Resource {
		...
		public Resource addPath(String path) throws IOException, MalformedURLException {
			if (path.startsWith("/ghost/framework/jetty/web/module")) {
				return null;
			}
