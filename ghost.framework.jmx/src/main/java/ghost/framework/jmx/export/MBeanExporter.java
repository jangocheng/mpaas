...
package ghost.framework.jmx.export;

import ghost.framework.beans.annotation.constraints.Nullable;
import ghost.framework.context.proxy.aop.autoproxy.target.LazyInitTargetSource;
import ghost.framework.jmx.export.assembler.AutodetectCapableMBeanInfoAssembler;
import ghost.framework.jmx.export.assembler.MBeanInfoAssembler;
import ghost.framework.jmx.export.assembler.SimpleReflectiveMBeanInfoAssembler;
import ghost.framework.jmx.export.naming.KeyNamingStrategy;
import ghost.framework.jmx.export.naming.ObjectNamingStrategy;
import ghost.framework.jmx.export.naming.SelfNaming;
import ghost.framework.jmx.export.notification.ModelMBeanNotificationPublisher;
import ghost.framework.jmx.export.notification.NotificationPublisherAware;
import ghost.framework.jmx.support.JmxUtils;
import ghost.framework.jmx.support.MBeanRegistrationSupport;
 ...
 * @see #setAssembler
 * @see #setListeners
 * @see ghost.framework.jmx.export.assembler.MBeanInfoAssembler
 * @see MBeanExporterListener
 */
public class MBeanExporter extends MBeanRegistrationSupport implements MBeanExportOperations, AutoCloseable {
...
//	private ListableBeanFactory beanFactory;
	 ...
	 * @see ghost.framework.jmx.export.naming.KeyNamingStrategy
...
//		this.autodetectMode = (Integer) constants.asNumber(constantName);
	 ...
	 * @see ghost.framework.jmx.export.assembler.SimpleReflectiveMBeanInfoAssembler
	 * @see ghost.framework.jmx.export.assembler.AutodetectCapableMBeanInfoAssembler
	 ...
	 * @see ghost.framework.jmx.export.naming.KeyNamingStrategy
	 * @see ghost.framework.jmx.export.naming.MetadataNamingStrategy
	 */
...
/*
		...
		if (isMBean(null)) {
			...
			Object proxy = proxyFactory.getProxy(this.beanClassLoader);
			*/
			ObjectName objectName = null;//getObjectName(proxy, beanKey);
			if (logger.isDebugEnabled()) {
				logger.debug("Located simple bean '" + beanKey + "': registering with JMX server as lazy-init MBean [" +
						objectName + "]");
			}
			ModelMBean mbean = null;//createAndConfigureMBean(proxy, beanKey);
	 ...
	 * @see ghost.framework.jmx.support.JmxUtils#isMBean(Class)
	 */
	...
	protected DynamicMBean adaptMBeanIfPossible(Object bean) throws JMException {
		Class<?> targetClass = null;//AopUtils.getTargetClass(bean);
...
//	}
	 ...
	 * {@link ghost.framework.jmx.export.notification.NotificationPublisher} is injected.
	 */
	 ...
	 * {@link ghost.framework.jmx.export.notification.NotificationPublisher}
