 ...
 * {@code ghost.framework.test} package, shipped in
...
public interface JdbcOperations {
	 ...
	 * mapped to a Map (one entry for each column, using the column value as the value).
	 * @param sql the SQL query to execute
	 * @return the result Map (one entry per column, with column value as value)
	 * @throws IncorrectResultSizeDataAccessException if the query does not
	 * return exactly one row
	 * @throws DataAccessException if there is any problem executing the query
	 ...
	 * {@code queryForList} method with {@code null} as argument array.
	 * <p>The results will be mapped to a List (one entry for each row) of
	 * Maps (one entry for each column using the column value as the value).
	 ...
	 * when you don't have a domain entity. Otherwise, consider using one of the
