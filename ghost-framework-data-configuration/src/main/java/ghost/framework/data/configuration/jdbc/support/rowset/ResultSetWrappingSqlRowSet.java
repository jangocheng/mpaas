 ...
 * the column value. Most JDBC ResultSet implementations follow this new pattern but there are
 * exceptions such as the {@code com.sun.rowset.CachedRowSetImpl} class which only uses
 * the column value, ignoring any column labels. As of Spring 3.0.5, ResultSetWrappingSqlRowSet
 ...
 * {@link ghost.framework.jdbc.core.JdbcTemplate} when working with RowSets.
 *
 ...
 * @see ghost.framework.jdbc.core.JdbcTemplate#queryForRowSet
 */
		...
		try {
			...
			if (rsmd != null) {
				...
				for (int i = 1; i <= columnCount; i++) {
					...
					// Make sure to preserve first matching column for any given value,
