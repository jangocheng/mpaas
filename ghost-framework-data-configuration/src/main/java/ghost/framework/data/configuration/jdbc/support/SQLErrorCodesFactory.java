...
import ghost.framework.util.PatternMatchUtils;
...
public class SQLErrorCodesFactory {
	...
	/**
	 * The value of custom SQL error codes file, loading from the root
	 * of the class path (e.g. from the "/WEB-INF/classes" directory).
	 */
	public static final String SQL_ERROR_CODE_OVERRIDE_PATH = "sql-error-codes.xml";

	/**
	 * The value of default SQL error code files, loading from the class path.
	 */
	 ...
	 * Key is the database product value, value is the SQLErrorCodes instance.
	 */
...
//	}
	 ...
	 * @param databaseName the database value (must not be {@code null})
	 * @return the {@code SQLErrorCodes} instance for the given database
	 * @throws IllegalArgumentException if the supplied database value is {@code null}
	 */
	public SQLErrorCodes getErrorCodes(String databaseName) {
		Assert.notNull(databaseName, "Database product value must not be null");
			...
			synchronized (this.dataSourceCache) {
				...
				if (sec == null) {
					...
					try {
						...
						logger.warn("Error while extracting database value - falling back to empty error codes", ex);
					}
	...
	/**
	 * Associate the specified database value with the given {@link DataSource}.
	 * @param dataSource the {@code DataSource} identifying the database
	 * @param databaseName the corresponding database value as stated in the error codes
	...
	public SQLErrorCodes registerDatabase(DataSource dataSource, String databaseName) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Caching SQL error codes for DataSource [" + identify(dataSource) +
					"]: database product value is '" + databaseName + "'");
		}
