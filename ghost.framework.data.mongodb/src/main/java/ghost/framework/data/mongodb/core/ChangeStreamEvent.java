...
import ghost.framework.data.mongodb.core.messaging.Message;
...
public class ChangeStreamEvent<T> {
	...
	public Instant getTimestamp() {
		...
		return getBsonTimestamp() != null ? (Instant)converter.getConverterContainer().convert(raw.getClusterTime(), Instant.class)
				: null;
	}
	...
	private Object doGetConverted(Document fullDocument) {
		...
		if (converter.getConverterContainer().canConvert(fullDocument.getClass(), targetType)) {

			result = converter.getConverterContainer().convert(fullDocument, targetType);
			return CONVERTED_UPDATER.compareAndSet(this, null, result) ? result : CONVERTED_UPDATER.get(this);
		}
