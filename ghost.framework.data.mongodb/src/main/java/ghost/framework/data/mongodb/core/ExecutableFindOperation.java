 ...
 * via {@link ghost.framework.data.mongodb.core.mapping.Document}. Using {@code inCollection} allows to override the
...
public interface ExecutableFindOperation {
	...
	interface TerminatingFindNear<T> {
		...
		/**
		 * Find all matching elements and return them as {@link ghost.framework.data.geo.GeoResult}.
		 *
		 * @return never {@literal null}.
		 */
		GeoResults<T> all();
	...
	interface DistinctWithProjection {
		 ...
		 * Any {@link org.bson.BsonType#DOCUMENT} is run through the {@link ghost.framework.data.commons.converter.EntityReader}
