...
import ghost.framework.data.transaction.NoTransactionException;
import ghost.framework.data.transaction.reactive.ReactiveResourceSynchronization;
...
import ghost.framework.data.transaction.support.ResourceHolderSynchronization;
...
public class ReactiveMongoDatabaseUtils {
	 ...
	 * {@link ReactiveTransactionSynchronization} has been registered for the
	...
	public static Mono<Boolean> isTransactionActive(ReactiveMongoDatabaseFactory databaseFactory) {
		...
		return ReactiveTransactionSynchronizationManager.forCurrentTransaction() //
				.map(it -> {
	 ...
	 * {@link Context} if {@link ReactiveTransactionSynchronizationManager#isSynchronizationActive() synchronization is active}.
	 *
	...
	private static Mono<MongoDatabase> doGetMongoDatabase(@Nullable String dbName, ReactiveMongoDatabaseFactory factory,
		...
		return ReactiveTransactionSynchronizationManager.forCurrentTransaction()
				.filter(ReactiveTransactionSynchronizationManager::isSynchronizationActive) //
	...
	private static Mono<ClientSession> doGetSession(ReactiveTransactionSynchronizationManager synchronizationManager,
	...
	private static class MongoSessionSynchronization
		...
		MongoSessionSynchronization(ReactiveTransactionSynchronizationManager synchronizationManager,
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.transaction.reactive.ReactiveResourceSynchronization#shouldReleaseBeforeCompletion()
		 */
		@Override
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.transaction.reactive.ReactiveResourceSynchronization#processResourceAfterCommit(java.lang.Object)
		 */
		@Override
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.transaction.reactive.ReactiveResourceSynchronization#afterCompletion(int)
		 */
		@Override
		public Mono<Void> afterCompletion(int status) {
			...
			return Mono.defer(() -> {
				...
				if (status == ReactiveTransactionSynchronization.STATUS_ROLLED_BACK && isTransactionActive(this.resourceHolder)) {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.transaction.reactive.ReactiveResourceSynchronization#releaseResource(java.lang.Object, java.lang.Object)
		 */
		@Override
