 ...
 */
package ghost.framework.data.mongodb.util.json;
	...
	static {
		...
		try {
			dateTimeHelper = loadDateTimeFormatter(
					"ghost.framework.data.mongodb.util.json.DateTimeFormatter$Java8DateTimeFormatter");
		} catch (LinkageError e) {
			// this is expected if running on a release prior to Java 8: fallback to JAXB.
			dateTimeHelper = loadDateTimeFormatter(
					"ghost.framework.data.mongodb.util.json.DateTimeFormatter$JaxbDateTimeFormatter");
		}
