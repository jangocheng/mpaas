...
import ghost.framework.data.commons.mapping.AssociationHandler;
import ghost.framework.data.commons.mapping.MappingException;
import ghost.framework.data.commons.mapping.PropertyHandler;
import ghost.framework.data.commons.mapping.model.BasicPersistentEntity;
import ghost.framework.data.commons.util.TypeInformation;
import ghost.framework.data.mongodb.MongoCollectionUtils;
...
import ghost.framework.expression.common.LiteralExpression;
...
public class BasicMongoPersistentEntity<T> extends BasicPersistentEntity<T, MongoPersistentProperty>
	...
	public BasicMongoPersistentEntity(TypeInformation<T> typeInformation) {
		...
		if (this.isAnnotationPresent(Document.class)) {
			...
			this.collection = StringUtils.hasText(document.value()) ? document.value() : fallback;
			this.language = StringUtils.hasText(document.language()) ? document.language() : "";
			this.expression = detectExpression(document.value());
			this.collation = document.value();
			this.collationExpression = detectExpression(document.value());
		} else {
	...
	private ShardKey detectShardKey() {
		...
		String[] keyProperties = sharded.value();
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mongodb.core.mapping.MongoPersistentEntity#getCollection()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mongodb.core.mapping.MongoPersistentEntity#getLanguage()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mongodb.core.mapping.MongoPersistentEntity#getTextScoreProperty()
	 */
	@Nullable
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mongodb.core.mapping.MongoPersistentEntity#hasTextScoreProperty()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mongodb.core.mapping.MongoPersistentEntity#getCollation()
	 */
	@Override
	public ghost.framework.data.mongodb.core.query.Collation getCollation() {
		...
		if (collationValue instanceof org.bson.Document) {
			return ghost.framework.data.mongodb.core.query.Collation.from((org.bson.Document) collationValue);
		}

		if (collationValue instanceof ghost.framework.data.mongodb.core.query.Collation) {
			return ghost.framework.data.mongodb.core.query.Collation.class.cast(collationValue);
		}

		return StringUtils.hasText(collationValue.toString())
				? ghost.framework.data.mongodb.core.query.Collation.parse(collationValue.toString())
				: null;
	}
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mapping.entity.BasicPersistentEntity#verify()
	 */
	@Override
	public void verify() {
	...
	/*
	 * (non-Javadoc)
	 * @see ghost.framework.data.mapping.entity.BasicPersistentEntity#getEvaluationContext(java.lang.Object)
	 */
	@Override
	...
	private static class PropertyTypeAssertionHandler implements PropertyHandler<MongoPersistentProperty> {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.mapping.PropertyHandler#doWithPersistentProperty(ghost.framework.data.mapping.PersistentProperty)
		 */
		@Override
		public void doWithPersistentProperty(MongoPersistentProperty persistentProperty) {
