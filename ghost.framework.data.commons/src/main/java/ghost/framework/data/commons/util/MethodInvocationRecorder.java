...
*/
			return null;//registerInvocation(method, rawType).getCurrentInstance();
		}
...
//	@Value
	private static class InvocationInformation {
		...
		static final InvocationInformation NOT_INVOKED = null;// new InvocationInformation(new Unrecorded(), null);
//		@NonNull
		Recorded<?> recorded;
		@Nullable
...
//					() -> Optional.of(propertyName));
	...
	private static enum DefaultPropertyNameDetectionStrategy implements PropertyNameDetectionStrategy {
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.hateoas.core.Recorder.PropertyNameDetectionStrategy#getPropertyName(java.lang.reflect.Method)
		 */
		@Nonnull
		@Override
...
//	@RequiredArgsConstructor
	public static class Recorded<T> {
		...
		private @Nullable MethodInvocationRecorder recorder;
		...
		/**
		 * Applies the given TypeConverter to the recorded value and remembers the property accessed.
		 *
		...
		public <S> Recorded<S> record(Function<? super T, S> converter) {
			...
			return null;//new Recorded<S>(converter.apply(currentInstance), recorder);
		}
		...
		public <S> Recorded<S> record(ToCollectionConverter<T, S> converter) {
			...
			return null;//new Recorded<S>(converter.apply(currentInstance).iterator().next(), recorder);
		}
		...
		public <S> Recorded<S> record(ToMapConverter<T, S> converter) {
			...
			return null;//new Recorded<S>(converter.apply(currentInstance).values().iterator().next(), recorder);
		}
...
//			super(null, null);
		...
		/*
		 * (non-Javadoc)
		 * @see ghost.framework.data.util.MethodInvocationRecorder.Recorded#getPropertyPath(java.util.List)
		 */
		@Override
		public Optional<String> getPropertyPath(List<PropertyNameDetectionStrategy> strategies) {
			return Optional.empty();
