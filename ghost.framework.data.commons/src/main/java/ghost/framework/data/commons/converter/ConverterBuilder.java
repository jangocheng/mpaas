...
/**
 * API to easily set up {@link GenericTypeDescriptorConverter} instances using Java 8 lambdas, mostly in bidirectional fashion for
...
public interface ConverterBuilder {
	...
	static <S, T> ReadingConverterBuilder<S, T> reading(Class<S> source, Class<T> target,
		...
		return new DefaultConverterBuilder<>(new GenericTypeDescriptorConverter.ConvertiblePair(source, target), Optional.empty(), Optional.of(function));
	}
	...
	static <S, T> WritingConverterBuilder<S, T> writing(Class<S> source, Class<T> target,
		...
		return new DefaultConverterBuilder<>(new GenericTypeDescriptorConverter.ConvertiblePair(target, source), Optional.of(function), Optional.empty());
	}

	/**
	 * Returns all {@link GenericTypeDescriptorConverter} instances to be registered for the current {@link ConverterBuilder}.
	 *
	 * @return
	 */
	Set<GenericTypeDescriptorConverter> getConverters();
	...
	interface WritingConverterAware {
		 ...
		 * @return
		 */
		GenericTypeDescriptorConverter getWritingConverter();
	}
	...
	interface ReadingConverterAware {
		 ...
		 * @return
		 */
		GenericTypeDescriptorConverter getReadingConverter();
	}
