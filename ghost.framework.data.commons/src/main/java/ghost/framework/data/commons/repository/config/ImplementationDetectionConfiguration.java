...
import ghost.framework.data.commons.repository.core.DefaultImplementationLookupConfiguration;
...
//import ghost.framework.data.jdbc.jpa.plugin.repository.core.DefaultImplementationLookupConfiguration;
...
public interface ImplementationDetectionConfiguration {
	...
	/**
	 * Generate the bean name for the given {@link IBeanDefinition}.
	 * 
	 * @param definition must not be {@literal null}.
	 * @return
	 */
	default String generateBeanName(IBeanDefinition definition) {
		...
		if (beanName == null) {
			throw new IllegalStateException("Cannot generate bean name for IBeanDefinition without bean class name!");
		}
	...
	default ImplementationLookupConfiguration forRepositoryConfiguration(RepositoryConfiguration<?> config) {
		...
		return new DefaultImplementationLookupConfiguration(this, config.getRepositoryInterface()) {
			/* 
			 * (non-Javadoc)
			 * @see ghost.framework.data.repository.config.DefaultImplementationLookupConfiguration#getBasePackages()
			 */
			@Override
