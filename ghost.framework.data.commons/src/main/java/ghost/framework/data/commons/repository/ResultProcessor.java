...
//import ghost.framework.core.CollectionFactory;
...
import ghost.framework.data.commons.repository.core.ParameterAccessor;
...
//import ghost.framework.data.jdbc.jpa.plugin.repository.query.ParameterAccessor;
...
public class ResultProcessor {
	 ...
	 */
	public ResultProcessor(QueryMethod method, ProjectionFactory factory) {
	...
	/**
	 * Post-processes the given query result using the given preparing {@link TypeConverter} to potentially prepare collection
	 * elements.
	 *
	 * @param source can be {@literal null}.
	 * @param preparingTypeConverter must not be {@literal null}.
	 * @return
	 */
	@Nullable
	@SuppressWarnings("unchecked")
	public <T> T processResult(@Nullable Object source, TypeConverter<Object, Object> preparingTypeConverter) {

		if (source == null || type.isInstance(source) || !type.isProjecting()) {
			return (T) source;
		}

		Assert.notNull(preparingTypeConverter, "Preparing converter must not be null!");

		ChainingConverter converter = ChainingConverter.of(type.getReturnedType(), preparingTypeConverter).and(this.converter);
	...
	private static class ChainingConverter implements TypeConverter<Object, Object> {

		private final Class<?> targetType;
		private final TypeConverter<Object, Object> delegate;

		private ChainingConverter(Class<?> targetType, TypeConverter<Object, Object> delegate) {
			this.targetType = targetType;
			this.delegate = delegate;
		}

		public static ChainingConverter of(Class<?> targetType, TypeConverter<Object, Object> delegate) {
		 ...
		 * given {@link TypeConverter}.
		 *
		 * @param typeConverter must not be {@literal null}.
		 * @return
		 */
		public ChainingConverter and(final TypeConverter<Object, Object> typeConverter) {
			...
			return new ChainingConverter(targetType, source -> {
				...
				return intermediate == null || targetType.isInstance(intermediate) ? intermediate
						: typeConverter.convert(intermediate);
			});
		}
	...
	/**
	 * A simple {@link TypeConverter} that will return the source value as is.
	 *
	 * @author Oliver Gierke
	 * @since 1.12
	 */
	private static enum NoOpConverter implements TypeConverter<Object, Object> {
	...
	private static class ProjectingConverter implements TypeConverter<Object, Object> {
		...
		ProjectingConverter(ReturnedType type, ProjectionFactory factory) {
			this(type, factory, null/*DefaultConverterContainer.getSharedInstance()*/);
		}

		public ProjectingConverter(ReturnedType type, ProjectionFactory factory, ConverterContainer conversionService) {
			this.type = type;
			this.factory = factory;
