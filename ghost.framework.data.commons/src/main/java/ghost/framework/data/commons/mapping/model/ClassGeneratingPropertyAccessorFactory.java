...
import ghost.framework.context.proxy.cglib.ReflectUtils;
...
import static ghost.framework.data.commons.mapping.model.BytecodeUtil.*;
...
public class ClassGeneratingPropertyAccessorFactory implements PersistentPropertyAccessorFactory {
	 ...
	 * 		Class class_1 = Class.forName("ghost.framework.data.mapping.Person");
	 * 		Class class_2 = Class.forName("ghost.framework.data.mapping.PersonWithId");
	...
	static class PropertyAccessorClassGenerator {
		...
		private static final String PERSISTENT_PROPERTY = "ghost/framework/data/mapping/PersistentProperty";
			...
			for (PersistentProperty<?> property : persistentProperties) {
				...
				if (property.isImmutable()) {
					if (generateMethodHandle(entity, property.getWither())) {
						cw.visitField(ACC_PRIVATE + ACC_FINAL + ACC_STATIC, witherName(property), referenceName(JAVA_LANG_INVOKE_METHOD_HANDLE), null, null).visitEnd();
					}
				} else {
		...
		private static void visitDefaultConstructor(PersistentEntity<?, ?> entity, String internalClassName,
			...
			mv.visitMethodInsn(INVOKESTATIC, "ghost/framework/util/Assert", "notNull",
