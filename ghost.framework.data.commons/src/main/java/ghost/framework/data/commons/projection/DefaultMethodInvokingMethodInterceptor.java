...
//		return getMethodHandle(method).bindTo(proxy).invokeWithArguments(arguments);
	...
	enum MethodHandleLookup {
		...
		ENCAPSULATED {
			...
			/*
			 * (non-Javadoc)
			 * @see ghost.framework.data.projection.DefaultMethodInvokingMethodInterceptor.MethodHandleLookup#lookup(java.lang.reflect.Method)
			 */
			@Override
			...
			/*
			 * (non-Javadoc)
			 * @see ghost.framework.data.projection.DefaultMethodInvokingMethodInterceptor.MethodHandleLookup#isAvailable()
			 */
			@Override
			boolean isAvailable() {
