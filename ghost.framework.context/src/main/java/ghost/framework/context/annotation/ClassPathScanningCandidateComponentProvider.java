...
import ghost.framework.beans.annotation.factory.Lookup;
 ...
 * {@link ghost.framework.core.type.classreading.MetadataReader MetadataReader}
 * facility, backed by an ASM {@link ghost.framework.asm.ClassReader ClassReader}.
 *
 * @author Mark Fisher
 ...
 * @since 2.5
 * @see ghost.framework.core.type.classreading.MetadataReaderFactory
...
//			this.environment = new StandardEnvironment();
	...
	/**
	 * Set the {@link IResourceLoader} to use for resource locations.
	 * This will typically be a {@link IResourcePatternResolver} implementation.
	 * <p>Default is a {@code PathMatchingIResourcePatternResolver}, also capable of
	 * resource pattern resolving through the {@code IResourcePatternResolver} interface.
	 ...
	 */
//	@Override
	public void setResourceLoader(@Nullable IResourceLoader IResourceLoader) {
	...
	/**
	 * Return the IResourceLoader that this component provider uses.
	 */
	public final IResourceLoader getResourceLoader() {
		return getResourcePatternResolver();
	}

	private IResourcePatternResolver getResourcePatternResolver() {
...
//			return AnnotationUtils.isAnnotationDeclaredLocally(Indexed.class, target);
	...
	private Set<IBeanDefinition> addCandidateComponentsFromIndex(CandidateComponentsIndex index, String basePackage) {
		Set<IBeanDefinition> candidates = new LinkedHashSet<>();
		try {
	...
	private Set<IBeanDefinition> scanCandidateComponents(String basePackage) {
		Set<IBeanDefinition> candidates = new LinkedHashSet<>();
		try {
			String packageSearchPath = IResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX +
					resolveBasePackage(basePackage) + '/' + this.resourcePattern;
			IResource[] resources = getResourcePatternResolver().getResources(packageSearchPath);
			boolean traceEnabled = logger.isTraceEnabled();
			boolean debugEnabled = logger.isDebugEnabled();
	...
	protected String resolveBasePackage(String basePackage) {
		return null;// ClassUtils.convertClassNameToResourcePath(getEnvironment().resolveRequiredPlaceholders(basePackage));
	}
