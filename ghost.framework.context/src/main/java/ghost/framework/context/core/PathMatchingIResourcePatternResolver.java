...
import ghost.framework.context.io.*;
...
/**
 * A {@link IResourcePatternResolver} implementation that is able to resolve a
 * specified resource location path into one or more matching Resources.
 * The source path may be a simple path which has a one-to-one mapping to a
 * target {@link ghost.framework.core.io.Resource}, or alternatively
 * may contain the special "{@code classpath*:}" prefix and/or
 * internal Ant-style regular expressions (matched using Spring's
 * {@link ghost.framework.util.AntPathMatcher} utility).
 ...
 * {@code getResource()} call on the underlying {@code IResourceLoader}.
 * Examples are real URLs such as "{@code file:C:/context.xml}", pseudo-URLs
 * such as "{@code classpath:/context.xml}", and simple unprefixed paths
 * such as "{@code /WEB-INF/context.xml}". The latter will resolve in a
 * fashion specific to the underlying {@code IResourceLoader} (e.g.
 ...
 * implicitly because the base {@code IResourceLoader} is a filesystem one,
 ...
 * This {@code IResourcePatternResolver} implementation is trying to mitigate the
 ...
 * @see ghost.framework.util.AntPathMatcher
 * @see ghost.framework.core.io.IResourceLoader#getResource(String)
 * @see ClassLoader#getResources(String)
 */
public class PathMatchingIResourcePatternResolver implements IResourcePatternResolver {

	private static final Log logger = LogFactory.getLog(PathMatchingIResourcePatternResolver.class);
	...
	static {
		try {
			...
			Class<?> fileLocatorClass = ClassUtils.forName("org.eclipse.core.runtime.FileLocator",
					PathMatchingIResourcePatternResolver.class.getClassLoader());
	...
	/**
	 * Create a new PathMatchingIResourcePatternResolver with a DefaultIResourceLoader.
	 * <p>ClassLoader access will happen via the thread context class loader.
	 * @see ghost.framework.core.io.DefaultResourceLoader
	 */
	public PathMatchingIResourcePatternResolver() {
		this.IResourceLoader = new DefaultIResourceLoader();
	}

	/**
	 * Create a new PathMatchingIResourcePatternResolver.
	 * <p>ClassLoader access will happen via the thread context class loader.
	 * @param IResourceLoader the IResourceLoader to load root directories and
	 * actual resources with
	 */
	public PathMatchingIResourcePatternResolver(IResourceLoader IResourceLoader) {
	 ...
	 * @see ghost.framework.core.io.DefaultResourceLoader
	 */
	public PathMatchingIResourcePatternResolver(@Nullable ClassLoader classLoader) {
		this.IResourceLoader = new DefaultIResourceLoader(classLoader);
	}


	/**
	 * Return the IResourceLoader that this pattern resolver works with.
	 */
	...
	public ClassLoader getClassLoader() {
		return getIResourceLoader().getClassLoader();
	}
	...
	public IResource getResource(String location) {
		return getIResourceLoader().getResource(location);
	}
	...
	public IResource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {
				...
				return new IResource[] {getIResourceLoader().getResource(locationPattern)};
			}
		}
	}
	...
	protected IResource[] findAllClassPathResources(String location) throws IOException {
		...
		Set<IResource> result = doFindAllClassPathResources(path);
	 ...
	 * @since 4.1.1
	 */
	protected Set<IResource> doFindAllClassPathResources(String path) throws IOException {
		Set<IResource> result = new LinkedHashSet<>(16);
	...
	/**
	 * Convert the given URL as returned from the ClassLoader into a {@link IResource}.
	 ...
	 * @since 4.1.1
	 */
	protected void addAllClassLoaderJarRoots(@Nullable ClassLoader classLoader, Set<IResource> result) {
	 ...
	 * @since 4.3
	 */
	protected void addClassPathManifestEntries(Set<IResource> result) {
		try {
	 ...
	 */
	private boolean hasDuplicate(String filePath, Set<IResource> result) {
		if (result.isEmpty()) {
			return false;
		}
	 ...
	 * @see ghost.framework.util.PathMatcher
	 */
	protected IResource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (IResource rootDirResource : rootDirResources) {
			...
			URL rootDirUrl = null;//rootDirResource.getURL();
	 ...
	 * @see #doFindPathMatchingJarResources
	 * @see ghost.framework.util.ResourceUtils#isJarURL
	 */
	 ...
	 * @see ghost.framework.util.PathMatcher
	 */
	protected Set<IResource> doFindPathMatchingJarResources(IResource rootDirResource, URL rootDirURL, String subPattern)
			throws IOException {
		...
		try {
			...
			Set<IResource> result = new LinkedHashSet<>(8);
...
//						result.add(rootDirResource.createRelative(relativePath));
	 ...
	 * @see ghost.framework.util.PathMatcher
	 */
	protected Set<IResource> doFindPathMatchingFileResources(IResource rootDirResource, String subPattern)
			throws IOException {
	...
	protected Set<IResource> doFindMatchingFileSystemResources(File rootDir, String subPattern) throws IOException {
		...
		Set<IResource> result = new LinkedHashSet<>(matchingFiles.size());
	...
	private static class VfsResourceMatchingDelegate {
		...
		public static Set<IResource> findMatchingResources(
	...
	private static class PatternVirtualFileVisitor implements InvocationHandler {
		...
		private final Set<IResource> resources = new LinkedHashSet<>();
