...
package ghost.framework.context.parsing;


import ghost.framework.context.bean.BeanMetadataElement;
import ghost.framework.context.bean.IBeanDefinition;
import ghost.framework.context.factory.BeanReference;

/**
 * Interface that describes the logical view of a set of {@link IBeanDefinition BeanDefinitions}
 * and {@link BeanReference BeanReferences} as presented in some configuration context.
 *
 * <p>With the introduction of {@link ghost.framework.beans.factory.xml.NamespaceHandler pluggable custom XML tags},
 * it is now possible for a single logical configuration entity, in this case an XML tag, to
 * create multiple {@link IBeanDefinition BeanDefinitions} and {@link BeanReference RuntimeBeanReferences}
 * in order to provide more succinct configuration and greater convenience to end users. As such, it can
 * no longer be assumed that each configuration entity (e.g. XML tag) maps to one {@link IBeanDefinition}.
 * For tool vendors and other users who wish to present visualization or support for configuring Spring
 * applications it is important that there is some mechanism in place to tie the {@link IBeanDefinition BeanDefinitions}
 * in the {@link ghost.framework.beans.factory.BeanFactory} back to the configuration data in a way
 * that has concrete meaning to the end user. As such, {@link ghost.framework.beans.factory.xml.NamespaceHandler}
 ...
 * supplied configuration information. In addition to this, each {@link IBeanDefinition} enclosed in a
 * {@code ComponentDefinition} has its own {@link IBeanDefinition#getSource() source object} which may point
 * to a different, more specific, set of configuration data. Beyond this, individual pieces of bean metadata such
 * as the {@link ghost.framework.beans.PropertyValue PropertyValues} may also have a source object giving an
 ...
 * {@link #getBeanReferences}, tools may wish to inspect all {@link IBeanDefinition BeanDefinitions} to gather
 ...
 * {@link IBeanDefinition BeanDefinitions} but this is not considered to be a typical case.
 *
 * <p>Tools can determine the important of contained {@link IBeanDefinition BeanDefinitions} by checking the
 * {@link IBeanDefinition#getRole role identifier}. The role is essentially a hint to the tool as to how
 * important the configuration provider believes a {@link IBeanDefinition} is to the end user. It is expected
 * that tools will <strong>not</strong> display all {@link IBeanDefinition BeanDefinitions} for a given
 ...
 * {@link IBeanDefinition#ROLE_INFRASTRUCTURE INFRASTRUCTURE role identifier}. {@link IBeanDefinition BeanDefinitions}
...
public interface ComponentDefinition extends BeanMetadataElement {
	...
	/**
	 * Return the {@link IBeanDefinition BeanDefinitions} that were registered
	 * to form this {@code ComponentDefinition}.
	 * <p>It should be noted that a {@code ComponentDefinition} may well be related with
	 * other {@link IBeanDefinition BeanDefinitions} via {@link BeanReference references},
	...
	/**
	 * Return the {@link IBeanDefinition BeanDefinitions} that represent all relevant
	 * inner beans within this component.
	 * <p>Other inner beans may exist within the associated {@link IBeanDefinition BeanDefinitions},
	 * however these are not considered to be needed for validation or for user visualization.
	 * @return the array of BeanDefinitions, or an empty array if none
	 */
	IBeanDefinition[] getInnerBeanDefinitions();
	 ...
	 * {@link IBeanDefinition BeanDefinitions}, however these are not considered
