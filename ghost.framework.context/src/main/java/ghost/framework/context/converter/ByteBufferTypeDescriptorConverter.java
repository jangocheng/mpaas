 ...
 * to any type that the {@link ConverterContainer} support via {@code byte[]}.
 *
 * @author Phillip Webb
 * @author Juergen Hoeller
 * @since 4.0
 */
public final class ByteBufferTypeDescriptorConverter implements ConditionalGenericTypeDescriptorConverter {
	...
	public ByteBufferTypeDescriptorConverter(@Autowired ConverterContainer converterContainer) {
	...
	private boolean matchesFromByteBuffer(TypeDescriptor targetType) {
		return (targetType.isAssignableTo(BYTE_ARRAY_TYPE) ||
				this.converterContainer.canConvert(BYTE_ARRAY_TYPE, targetType));
	}

	private boolean matchesToByteBuffer(TypeDescriptor sourceType) {
		return (sourceType.isAssignableTo(BYTE_ARRAY_TYPE) ||
				this.converterContainer.canConvert(sourceType, BYTE_ARRAY_TYPE));
	}
	...
	private Object convertFromByteBuffer(ByteBuffer source, TypeDescriptor targetType) {
		...
		return this.converterContainer.convert(bytes, BYTE_ARRAY_TYPE, targetType);
	}
	...
	private Object convertToByteBuffer(@Nullable Object source, TypeDescriptor sourceType) {
		byte[] bytes = (byte[]) (source instanceof byte[] ? source :
				this.converterContainer.convert(source, sourceType, BYTE_ARRAY_TYPE));
