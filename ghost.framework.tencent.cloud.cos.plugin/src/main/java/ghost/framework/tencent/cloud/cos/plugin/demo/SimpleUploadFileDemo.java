package ghost.framework.tencent.cloud.cos.plugin.demo;
...
public class SimpleUploadFileDemo {
    ...
    public static void SimpleUploadFileFromLocal(boolean useTrafficLimit) {
        ...
        // bucket名需包含appid
        String bucketName = "2424-1258196692";

        String key = "aaa/bbb.txt";
        File localFile = new File("src/test/resources/len10M.txt");
        PutObjectRequest putObjectRequest = new PutObjectRequest(bucketName, key, localFile);
        try (InputStream stream = SimpleUploadFileDemo.class.getResourceAsStream("/jpg.jpg")) {
            putObjectRequest = new PutObjectRequest(bucketName, UUID.randomUUID().toString() + ".jpg", stream, null);
            //
            if (useTrafficLimit) {
